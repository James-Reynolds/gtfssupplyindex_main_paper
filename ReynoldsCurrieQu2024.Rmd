...
# Introduction
Social equity concerns support the provision of at least a basic level of 
transit service in many locations, so as to enable those who cannot otherwise drive themselves to have at least some mobility and accessibility 
to activities and service beyond walking distance [@Currie:2016aa].
Age, 
disability, 
socio-economic status, 
lack of a driver's license 
or vehicle, or other factors 
might make someone
reliant on 
transit for some or all of their travel. 
Social-equity perspectives on transport policy-making 
related to vertical equity and supporting those who are disadvantaged 
(c.f. @Litman:2016aa) 
...
@Currie2003Hobart, 
@Currie2004Gap; @Currie2007Identifying; @currie2010identifying developed an approach for identifying 
spatial gaps in transit supply related to social needs for transport, and  applied this to a case study of Melbourne in 2006 (in @Currie2007Identifying; @currie2010identifying). This approach was suggested as being "substantially more useful than the presentation of anecdotal evidence, 
...
although there are a large number of metrics 
...
```{r Currie_map_SI, fig.ncol = 2, out.width = "50%", fig.align = "center", fig.cap = "Melbourne 2006", fig.subcap = c("Distribution of Transport Supply", "Distibution of Social Need for Transport", "CCDs with Very High needs and Very Low or Zero supply", "Supply Index scores plotted against Composite Needs Index scores"),  echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE}
...
Case research approaches can be particularly useful research questions are about 'how' or 'why', but when researchers do not have control of events and so cannot use experimental approaches [@Yin2009aa]. In this research study questions relate to (1) how to use GTFS data to assess gaps between social needs and transit supply, and (2) how and why spatial patterns might have changed since those reported for 2006 in @currie2010identifying. There is no ability here to control events related to spatial patterns of social needs and gaps in transit supply, so a case research approach appears well suited to this study. 

There is a need to addressing the "duality criterion" when using a case research approach, this being the need of the research to be seeking generalisable findings while at the same time being grounded in the context of only a small number of cases [@Denscombe2007aa; @Ketokivi2014aa]. Here the approach taken has been to develop a package of tools 
for calculating the SI from GTFS data 
using the R programming language [@R-base]. Using these methods addresses the duality criterion for the first research question, as the developed software functions are generic. They are applicable to other GTFS feeds, and could be used to analyse spatial gaps in transit supply in other places (and by others), beyond the Melbourne case reported in this paper.
...
This was sourced via the absmapsdata package [@absmapsdata].
...
Statistical Area 1 (SA1) zones. As such, SI scores have also been calculated for SA1s to facilitate the use of ABS data. The same Transport Supply categorizations have been used as in @currie2010identifying (Zero, Very Low, Low, Below average, Above average, High and Very High). 
...
#        legend.key.size = unit(0.08, 'cm')
  ...
  scale_fill_manual(values = c("black", "#F9D8B1", "#F7C387", "#F29C33", "#5DB000", "#53A212", "#407F0B")) 
...
Table \ref{tab:Greater_Melbourne_CCDs_SA1_table} summarises the distribution of CCDs and SA1s across different Transport Supply categories in 2006, 2016 and 2021. Figure \ref{fig:Greater_Melbourne_CCDs_SA1_population} shows the spatial distribution of Transport Supply in 2021. Maps for 2016 and CCDs are included in the Appendix. 
...
pcol <- pop_comparison %>% 
  ...
  geom_text(aes(transit_supply, population + 125000, label = glue::glue("{label_percent(accuracy = 0.1)(perc)}")), size = 2) +
  coord_flip() +
  scale_fill_manual(values = c("black", "#F9D8B1", "#F7C387", "#F29C33", "#5DB000", "#53A212", "#407F0B")) +
...
```
 ...
 @currie2010identifying reported an average SI score for CCDs in 2006 of 2,886.9 across Melbourne. The average SI value in 2021 (using CCDs and within the 2006 boundary) was `r si_CCD2006_21_census_week_aggregated[["si_by_area"]] %>% select(SI) %>% st_drop_geometry() %>% unlist()%>% mean() %>% format(big.mark = ",", digits = 5)`. SI scores average `r si_CCD2006_21_census_week_aggregated$si_by_area %>% st_drop_geometry() %>% filter(suburbs == "inner") %>% select(SI) %>% unlist() %>% mean() %>% format(big.mark = ",", digits = 6)`, `r si_CCD2006_21_census_week_aggregated$si_by_area %>% st_drop_geometry() %>% filter(suburbs == "middle") %>% select(SI) %>% unlist() %>% mean() %>% format(big.mark = ",", digits = 5)` and `r si_CCD2006_21_census_week_aggregated$si_by_area %>% st_drop_geometry() %>% filter(suburbs == "outer") %>% select(SI) %>% unlist() %>% mean() %>% format(big.mark = ",", digits = 5)` in 2021 for the CCDs in the inner, middle and outer suburbs respectively^[The same grouping of LGAs to inner, middle and outer suburb groups as used in @currie2010identifying was used for this analysis, although here the City of Stonnington was allocated entirely to the middle grouping, whereas @currie2010identifying allocated part of this LGA to the inner group.], compared to 10,922.7, 2,694.9 and 764.3, respectively, reported for 2006 in @currie2010identifying. However, the @currie2010identifying show 5,839 total CCDs within Melbourne, whereas the shape files obtained for this analysis include `r si_CCD2006_16_census_week_aggregated$si_by_area %>% st_drop_geometry() %>% nrow() %>% format(big.mark = ",")` CCDs within the 2006 Melbourne boundary. Hence, results found here might not be exactly comparable to those in @currie2010identifying due to geographic inconsistencies, although overall results appear to be consistent with there having been increases in SI scores across all parts of Melbourne since 2006 and the general pattern of higher average SI scores in inner and then middle suburbs remaining unchanged. 
...
# <20% are less than 5, but "Service withdrawn (black)" is all less than 5. So check without sevice withdrawn 

# drop Service withdrawn
#si_by_area_2016_2021_wider_no_withdrawn <- si_by_area_2016_2021_wider %>% filter(ratio_binned != "Service withdrawn (black)")
...
```{r Greater_Melbourne_2016_2021_ratio_map, fig.show="hold", fig.dim = c(7.5,7.5),  echo = FALSE, warning=FALSE, message=FALSE, cache = TRUE, include = TRUE, fig.cap="Greater Melbourne: changes in SI between 2016 and 2021, by 2021 SA1, overlayed with SA4 boundaries (blue) and suburban railway lines (black)"}
...
Greater_Melbourne_2016_2021_ratio_map <- ggplot()+ 
  geom_sf(data=si_by_area_2016_2021_wider %>% 
            na.omit() %>%
            filter(ratio_binned != "Never served"),
          aes(fill = ratio_binned), colour=NA) +  scale_fill_manual(values = c("#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "black", "magenta")) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
        axis.ticks.y=element_blank(), #remove y axis ticks
...
pcol <- si_by_area_2016_2021_wider %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, population + 100000, label = glue::glue("{label_percent(accuracy = 0.1)(perc)}")), size = 2) +
  coord_flip() + 
  scale_fill_manual(values = c( "#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "black", "magenta"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
...
Greater_Melbourne_2016_2021_ratio_map + 
  inset_element(pcol, left = 0.0, bottom = 0.1, right = 0.45, top =0.38)
...
It is, however, possible to directly compare SI scores 2016 and 2021. Average SI scores for all of Greater Melbourne (SA1 2021 boundaries) increased from `r si_by_area_2016_2021_wider$x2016 %>% na.omit() %>% mean() %>% format(big.mark = ",", digits = 5)` in 2016 to `r si_by_area_2016_2021_wider$x2021 %>% na.omit() %>% mean() %>% format(big.mark = ",", digits = 5)` in 2021 (+`r percent_format(accuracy = 0.1)(si_by_area_2016_2021_wider$x2021 %>% na.omit() %>% mean()/si_by_area_2016_2021_wider$x2016 %>% na.omit() %>%  mean()-1)`). There is a statistically significant and strong correlation between the 2016 and 2021 SI scores (`r prmisc::print_cortest(cor.test(si_by_area_2016_2021_wider$x2016, si_by_area_2016_2021_wider$x2021, method = "pearson"))`, `r prmisc::print_cortest(cor.test(si_by_area_2016_2021_wider$x2016, si_by_area_2016_2021_wider$x2021, method = "spearman", exact = FALSE))`).

Table \ref{tab:Greater_Melbourne_2016_2021_ratio_map} shows changes in SI score between 2016 and 2021 (column 1) by 2021 SA1s for the 2021 population, summarised for each SA4 zone (columns 2 to 10) and Greater Melbourne as a whole (column 11). These are mapped in Figure \ref{fig:Greater_Melbourne_2016_2021_ratio_map}.There is a statistically significant difference in the distribution of service level changes across the populations between SA4 areas (`r prmisc::print_chi2(greater_melbourne_ratio_population_wider %>% select(Inner, "Inner East", "Inner South", "North East", "North West", "Outer East", "South East", "West", "Mornington Peninsula")  %>% as.matrix())`). In 2021, more than half of the population were living in SA1s where the SI scores were more than one percent higher than in 2016 in the North-West (`r greater_melbourne_ratio_population_wider[2:7,5] %>% sum() %>% format(big.mark = ",")` people, `r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[2:7,5] %>% sum() / greater_melbourne_ratio_population_wider[,5] %>% sum())`), West (`r greater_melbourne_ratio_population_wider[2:7,8] %>% sum() %>% format(big.mark = ",")` people, `r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[2:7,8] %>% sum() / greater_melbourne_ratio_population_wider[,8] %>% sum())`), North East (`r greater_melbourne_ratio_population_wider[2:7,4] %>% sum() %>% format(big.mark = ",")` people, `r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[2:7,4] %>% sum() / greater_melbourne_ratio_population_wider[,4] %>% sum())`), South East (`r greater_melbourne_ratio_population_wider[2:7,7] %>% sum() %>% format(big.mark = ",")` people, `r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[2:7,7] %>% sum() / greater_melbourne_ratio_population_wider[,8] %>% sum())`), Mornington Peninsula (`r greater_melbourne_ratio_population_wider[2:7,9] %>% sum() %>% format(big.mark = ",")` people, `r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[2:7,9] %>% sum() / greater_melbourne_ratio_population_wider[,9] %>% sum())`) and Inner (`r greater_melbourne_ratio_population_wider[2:7,10] %>% sum() %>% format(big.mark = ",")` people, `r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[2:7,10] %>% sum() / greater_melbourne_ratio_population_wider[,10] %>% sum())`) SA4s 


Service levels in 2021 were within one percent of the level in 2016 for most of the population in the Outer East (`r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[8,6] %>% sum() /sum(greater_melbourne_ratio_population_wider[,6]))`) and Inner East (`r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[8,2] %>% as.numeric() /sum(greater_melbourne_ratio_population_wider[,2]))`), compared to only `r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[8,2:10] %>% sum() /sum(greater_melbourne_ratio_population_wider[,2:10]))` of the population across all of Greater Melbourne. The largest shares of the population living in areas were service levels remained within one percent were in the Outer East (`r greater_melbourne_ratio_population_wider[8,6] %>% as.numeric() %>% format(big.mark = ",")` people, `r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[8,6] %>% as.numeric() / greater_melbourne_ratio_population_wider[8,2:10] %>% as.numeric() %>% sum())` of the `r greater_melbourne_ratio_population_wider[8,2:10] %>% as.numeric() %>% sum() %>% format(big.mark = ",")`), South East (`r greater_melbourne_ratio_population_wider[8,7] %>% as.numeric() %>% format(big.mark = ",")` people, `r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[8,7] %>% as.numeric() / greater_melbourne_ratio_population_wider[8,2:10] %>% sum())`) and Inner East (`r greater_melbourne_ratio_population_wider[8,2] %>% as.numeric() %>% format(big.mark = ",")` people, `r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[8,2] %>% as.numeric() / greater_melbourne_ratio_population_wider[8,2:10] %>% sum())`).



In 2021, the share of the population living in SA1s where the SI was one or more percent lower than in 2016 was largest in the Inner (`r label_percent(accuracy = 0.1)(sum(greater_melbourne_ratio_population_wider[9:12,10]) /sum(greater_melbourne_ratio_population_wider[,10]))`), Inner South (`r label_percent(accuracy = 0.1)(sum(greater_melbourne_ratio_population_wider[9:12,3]) /sum(greater_melbourne_ratio_population_wider[,3]))`) and Outer East (`r label_percent(accuracy = 0.1)(sum(greater_melbourne_ratio_population_wider[9:12,6]) /sum(greater_melbourne_ratio_population_wider[,6]))`) SA4s. The largest shares of residents with at less one percent lower service level in 2021 were among the Inner (`r sum(greater_melbourne_ratio_population_wider[9:12,10]) %>% format(big.mark = ",")`), Outer East (`r sum(greater_melbourne_ratio_population_wider[9:12,6]) %>% format(big.mark = ",")`) and South East (`r sum(greater_melbourne_ratio_population_wider[9:12,7]) %>% format(big.mark = ",")`) SA4 areas.   
...
```{r Greater_Melbourne_2021_needs_gap_scatterplot, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, fig.height=8, fig.cap="Greater Melbourne 2021, SI and Combined Needs Index scores, with SI scores < 10 rounded up to equal 10."}
...
```{r Greater_Melbourne_2016_needs_gap_map_figure, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, out.width="100%", fig.cap="Greater Melbourne 2016 SI groupings, overlayed with SA1s with very high transport need areas with zero or very low public transport supply (red)."}
...
```{r Greater_Melbourne_2021_needs_gap_map_figure, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, out.width="100%", fig.cap="Greater Melbourne 2021 SI groupings, overlayed with SA1s with very high transport need areas with zero or very low public transport supply (red)."}
...
scale_fill_manual(values = c("#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "black", "magenta")) +
  ...
  ggrepel::geom_label_repel(
    data = si_by_area_2016_2021_wider %>% 
      na.omit() %>% 
      filter(very_high_needs_zero_or_very_low_supply == TRUE) %>% 
      filter(ratio_binned == "Service withdrawn (black)"),
...
```{r Bus_798, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.ncol = 2, out.width = "50%", fig.align = "center", fig.show="hold", fig.cap="Bus Route 798 as originally introduced in 2014 (Source: Delbosc et al (2015))"}
 ...
 coord_sf(ylim = c(-38.07, -38.16), 
  ...
  ggrepel::geom_label_repel(
    data = si_by_area_2016_2021_wider %>% 
      na.omit() %>% 
      filter(very_high_needs_zero_or_very_low_supply == TRUE) %>% 
      filter(ratio_binned == "Never served" | ratio_binned == "Service withdrawn (black)"),
    aes(label = population, geometry = geometry),
    stat = "sf_coordinates",
    min.segment.length = 0 )  +
  theme(legend.position = "none", 
