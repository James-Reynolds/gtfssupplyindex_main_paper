...
pcol <- pop_comparison %>% 
  ...
  geom_text(aes(transit_supply, population + 80000, label = glue::glue("{label_percent(accuracy = 0.1)(perc)}")), size = 2) +
  coord_flip() +
  scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "#5DB000", "#53A212", "#407F0B")) +
...
`r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[7,9] %>% as.numeric() /sum(greater_melbourne_ratio_population_wider[,9]))`), 
compared to slightly more than one quarter 
...
of the population across all of Greater Melbourne. 
