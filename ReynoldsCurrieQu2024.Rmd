...
abstract: |
  Transit often provides an vital social purpose, linking those who cannot otherwise drive to services, activities and community. Vertical social equity perspecties on transport policy emphasise providing services to those most in need. In previous research Currie and Senbergs (2007) and Currie (2010) developed an analysis approach for identifying spatial areas where gaps between transit supply, accounting for frequency and coverage, and transport needs are larger. 
  However, this approach does not appear to have been widely adopted or received further research attention. This may be due to the challenges associated with assessing transit supply levels prior to the widespread avaialbility of schedule data in electronic formats. Nowadays, GTFS datasets are provided by many operators, allowing the development of standardised processing tools. Such a tool, though, does not yet appears to be available for  needs-gap analysis, nor does it appear that there has been further review of spatial patterns since the Currie and Senbergs (2007) and Currie (2010) case study of Melbourne in 2006. 
  This paper reportes the development of a software package with tools for using GTFS dataset for needs-gap analysi, available for the R statistiacal analysis language. The paper also reports results for Melbourne in 2016 and 2021, both tot demostrate the package capabilities and continue the previous research as a longitudinal case study.  
  Results indicate similar patterns of social need and transit supply as in 2006, with lower supply levels in outer areas, particularly those that are away from the suburban rail network. The share of residents living in areas with Very High needs but Very Low or Zero transit supply, however, has increased from 8.2% of the population in 2006 to 11.2% in 2016 and ?? in 2021. The paper also briefly revists a case study of early-2010s development in the outer south east reported in Delbosc et al (2015), finding that there have been positive changes in service level at this local scale, but needs-gaps for parts of the neighbourhood are amongst the highest in Greater Melbourne. While Melbourne may be an outlier in terms of high population growth (3.4m to 4.9m in 15 years) at generally low densities, compared to other cities, theses result might suggest that providing even a basic level of transit-based movility to those in need has only become more difficult. Direction for further research and limitations are also identified.  
...
in many locations
so those who cannot otherwise drive themselves
have at least some independent
and motorised mobility 
[@Currie:2016aa]. 
Age, 
disability, 
...
For these people
even an infrequent transit service
...
relate to supporting those who are disadvantaged [@Litman:2016aa] 
and might suggest providing at least some transit, 
...
for comparison to the 2006 results reported in @currie2010identifying[^1].
...
especially for those who are not planners, engineers or other technical specialists. 
...
which provides metrics for many aspects of a transit system that can be independently calculated (with sufficient data).

GTFS datasets have allowed the application of such metrics to many transit systems. 
The *Transit Score* website itself provides one example, 
providing scores for places publishing a data feed.
...
The approach outlined in @currie2010identifying involved calculating scores 
for geographical areas of interest across 
a Transit Supply Index (SI) 
and a Composite Social Needs Index. 
These were then used to identify areas with Very High needs, 
but Very Low or Zero transit supply.  

The SI incorporates both the frequency of transit services and their proximity. 
...
These were mostly in the outer suburbs. 
...
The case-based methods used in this research 
...
given that the tourist railway is located in the south-east of Melbourne, 
the authors are familiar with the surrounding geography 
...
provides guidance on case selection 
and discusses various theoretical sampling approaches, 
including sampling for critical, 
particularly revelatory 
and/or representative cases [@Eisenhardt1989aa; @Yin2009aa; @Denscombe2007aa; @Eisenhardt2007TBfC]. 
@Yin2009aa notes selection of a case to allow longitudinal study, 
and this is the primary reason for selecting Melbourne here,
...
but for the weeks starting the day of the 2016 and 2021 censuses.
...
ranging from SA1s 
(generally ranging from 200 to 800 residents, 
and averaging around 400 people)
...
Greater Melbourne's population increased by 
...
However, the number of residents living in areas with Zero Supply more than doubled
...
(`r label_percent(accuracy=0.1)(sum(melbourne_comparison_2006_2021[1:2,3])/sum(melbourne_comparison_2006_2021[,3]))` of the population)
in 2006 to 
...
and 2021
(`r prmisc::print_chi2(melbourne_2021_transit_supply_by_SA4 %>% pivot_wider(names_from = sa4_name_2021, values_from = SA1s) %>% select(Inner, "Inner East", "Inner South", "North East", "North West", "Outer East", "South East", "West", "Mornington Peninsula") %>% as.matrix())`)[^9].
Tables \ref{tab:Greater_Melbourne_population_2016_by_SA4}
and \ref{tab:Greater_Melbourne_population_2021_by_SA4}
show the population in each supply category by SA4 for 2016 and 2021.
...
However, average scores found here might not be exactly comparable
...
while the general pattern of higher average SI scores
in inner (and then middle) suburbs than in outer suburbs remains unchanged.
...
```{r Greater_Melbourne_2016_2021_ratio_map_plot, fig.show="hold", fig.dim=c(7.5,7.5),  echo = FALSE, warning=FALSE, message=FALSE, cache = FALSE, include = TRUE, fig.cap="Greater Melbourne: changes in SI between 2016 and 2021, by 2021 SA1, overlayed with 2006 Greater Melbourne boundary (black); 2021 SA4 boundaries (blue); and suburban railway lines (black)"}
 ...
 ggplot()+ 
  ...
  inset_element(pcol_inner_south, left = 0.33, bottom = 0.18, right = 0.63, top =0.33) + inset_element(pcol_outer_east, left = 0.72, bottom = 0.7, right = 1.02, top =0.85)  
...
categorised into those SA1s where service levels have reduced (3 groups),
stayed within 1% (1 group),
increased (5 groups),
and where transit services have been introduced,
were withdrawn
or were not provided in either 2016 or 2021.
...
The share of the population living where transit service had been withdrawn
...
inset_element(pcol, left = 0.0, bottom = 0.0, right = 0.4, top =0.30) +
  ...
  inset_element(pcol_south_east, left = 0.82, bottom = 0.04, right = 1.12, top =0.19) +
  inset_element(pcol_outer_east, left = 0.82, bottom = 0.60, right = 1.12, top =0.75) +
  inset_element(pcol_north_east, left = 0.59, bottom = 0.81, right = 0.89, top =0.96)  +
  inset_element(pcol_north_west, left = 0.0, bottom = 0.81, right = 0.3, top =0.96) +
  inset_element(pcol_west, left = 0.00, bottom = 0.30, right = 0.3, top =0.45) +
  inset_element(pcol_inner, left = 0.28, bottom = 0.27, right = 0.58, top =0.42) +
  inset_element(pcol_inner_south, left = 0.28, bottom = 0.12, right = 0.58, top =0.27) +  inset_element(pcol_inner_east, left = 0.75, bottom = 0.78, right = 1.05, top =0.93) 
...
although this may be an artifact of the shift to SA1s from CCDs[^15].
...
pcol_mornington <- left_join(data.frame(ratio_binned_collapsed=levels(si_by_areaSA12016_2016_2021_wider$ratio_binned_collapsed)),
 ...
 Greater_MelbourneSA12016_2016_2021_ratio_map +
  ggspatial::annotation_scale(location = 'br') + 
  ggspatial::annotation_north_arrow(location = 'tr') + 
  inset_element(pcol, left = 0.0, bottom = 0.15, right = 0.55, top =0.35) +
...
it is possible to see how much service levels changed by 2021 for those 
who had the highest needs, but least service in 2016. 
Table \ref{tab:Greater_Melbourne_2016_needs_gap_SA4_service_change}
shows this analysis by the 2016 population
rather than number of SA1s.

Overall, `r si_by_areaSA12016_2016_2021_wider_table[1:4,2:9] %>% sum() %>% format(big.mark = ",")` people (`r label_percent(accuracy = 1)(si_by_areaSA12016_2016_2021_wider_table[1:4,2:9] %>% sum()/sum(melbourne_comparison_2006_2021[,8]))`
of the Greater Melbourne population) lived in SA1s
with Very High needs but Very Low or Zero supply in 2016.  
By 2021, however, transit supply had been newly introduced
or had increased by at least 30% 
in SA1s where `r si_by_areaSA12016_2016_2021_wider_table[4,2:9] %>% sum() %>% format(big.mark = ",")`
of those people (`r label_percent(accuracy = 1)(si_by_areaSA12016_2016_2021_wider_table[4,2:9] %>% sum()/si_by_areaSA12016_2016_2021_wider_table[1:4,2:9] %>% sum())` 
had been living (in 2016).
A further `r si_by_areaSA12016_2016_2021_wider_table[3,2:9] %>% sum() %>% format(big.mark = ",")`
of those people (`r label_percent(accuracy = 1)(si_by_areaSA12016_2016_2021_wider_table[3,2:9] %>% sum()/si_by_areaSA12016_2016_2021_wider_table[1:4,2:9] %>% sum())` 
had been living in SA1s where the transit supply increased
by 1 to 30%,
and `r si_by_areaSA12016_2016_2021_wider_table[2,2:9] %>% sum() %>% format(big.mark = ",")`
of those people (`r label_percent(accuracy = 1)(si_by_areaSA12016_2016_2021_wider_table[2,2:9] %>% sum()/si_by_areaSA12016_2016_2021_wider_table[1:4,2:9] %>% sum())` 
were in SA1s
where the SI score was still within 1% of 
its 2016 level by 2021.  
For the SA1s where `r si_by_areaSA12016_2016_2021_wider_table[1,2:9] %>% sum() %>% format(big.mark = ",")` people
...
reduced to zero, 
or had never had transit and still didn't by 2021.  

There is a statistically significant variation
across the non-Inner SA4s^[Categories of change in service level
collapsed and the Inner, Inner East and Inner South SA4s
removed to meet assumptions of the chi-square test.].
...
therefore,
is important because it may help bridge gaps
between research
...
but these may be impossible to calculate independently 
or complicated for communication
with the public,
decision-makers
or other stakeholders.
The SI has the advantage of combining
accessibility
and service frequency
into a single measure,
which appears simple, 
and can even be calculated by hand
for smaller networks.
The gtfstools R package
developed in this research
may make it possible for practitioners
to apply the SI calculation
across even relatively large transit networks,
such as that of Greater Melbourne as shown here,
...
and easily demonstrate
to the public, politicians and other stakeholders
which spatial areas might benefit from increased transit
and where funds might best be directed.
The map shown in
Figure \ref{fig:Greater_Melbourne_2016_needs_gap_SA4_service_change}
demonstrates how the needs-gap analysis approach,
in conjunction with a comparison of service levels through time,
cam be used to show how things have changed
for those residents
who had Very High transport needs, but Very Low or Zero Supply.
Such maps might provide evidence for those living in areas with high needs,
but where service levels have moved in the wrong direction
(such as appears to be the case for the
...
a further `r (si_by_areaSA12016_2016_2021_wider_table[4,] %>% adorn_totals(where = "col") %>% select(Total) %>% sum() - sum(si_by_areaSA12016_2016_2021_wider_table[4,"North East"] + si_by_areaSA12016_2016_2021_wider_table[4,"West"] + si_by_areaSA12016_2016_2021_wider_table[4,"Mornington Peninsula"])) %>% format(big.mark = ",")`
people who were living elsewhere in Greater Melbourne in 2016
in SA1s with Very High needs and Very Low or Zero supply, 
but who saw their transit service either reduced or withdrawn by 2021,
or who never had any transit at all). 
  
The changes by 2021,
however,
appear to have been mostly positive for the
`r si_by_areaSA12016_2016_2021_wider_table[,"North East"] %>% sum() %>% format(big.mark = ",")`
people living in the South East SA4 in 2016
in SA1s with Very High needs and Very Low or Zero supply.
Table \ref{tab:Greater_Melbourne_2016_needs_gap_SA4_service_change}
indicates that
`r label_percent(accuracy = 1)(si_by_areaSA12016_2016_2021_wider_table[1:2,"South East"] %>% sum()/si_by_areaSA12016_2016_2021_wider_table[,"South East"] %>% sum())`)
...
while Figure \ref{fig:Greater_Melbourne_2016_needs_gap_SA4_service_change}
suggests that areas around Cranbourne have seen substaintial service increases (30%+)
or the introduction of new services (dark blue).
This part of Greater Melbourne has received previous research attention
from @delbosc2015impact,
who looked at bus services in new developments. 
This included a case study of the Selandra Rise estate,
east of Cranbourne, 
exploring the impact of the, then newly introduced, 798 bus route
...
despite it not penetrating fully into the development
and only 20 percent of the estate's footprint being within 400 metres of the route.
Findings suggested that: the route was
...
showing Transport Supply in 2016 and 2021,
changes in SI score, 
and social needs
and needs-gaps in 2021,
overlayed with transit routes.

```{r Bus_798, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.ncol = 2, out.width = "50%", fig.align = "center", fig.show="hold",  fig.cap="Cranbourne and Selandra Rise", fig.subcap=c("Bus Route 798 as originally introduced in 2014 (Source: Delbosc et al (2015))", "Surrounding transit routes in 2018", "Surrounding transit routes in 2021", "Surrounding transit routes in 2024 (Source: PTV)")}
...
```{r Selandra_rise, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.ncol = 2, fig.height=1.3, fig.width=3.8,fig.align = "center", fig.show="hold", fig.cap="Cranbourne and the Selandra Rise Estate, overlayed with transit routes (purple)", fig.subcap = c("Transport Supply by SA1: 2016", "Transport Supply by population: 2016", "Transport Supply by SA1: 2021", "Transport Supply by population: 2021", "Change in SI score 2016 to 2021, by SA1", "Change in SI score 2016 to 2021, by 2021 population", "Social Need by SA1: 2021", "Social Need by population: 2021", "Needs-gap: SA1s with Very High needs 2021, by supply", "Needs-gap: population with Very High 2021, by supply")}
...
si_by_area_2016_2021_wider %>%
 ...
 geom_col(aes(ratio_binned, perc, fill = ratio_binned), colour="black") +
  ...
  scale_y_continuous(limits = c(0,0.7)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, perc+0.04, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 2.5, colour = "black") +
   coord_flip() +
  scale_fill_manual(values = c("#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "magenta", "black")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
...
combined_needs_index_2021_sa1_greater_melbourne %>%
 ...
 # scale_x_discrete(drop=FALSE) +
  scale_y_continuous(limits = c(0,0.7)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs, perc+0.04, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 2.5) +
   coord_flip() +
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
#  geom_sf(data=si_by_areaSA12016_2016_2021_wider %>% 
#            filter(very_high_needs_zero_or_very_low_supply == TRUE),
#          aes(fill = ratio_binned_collapsed), colour="black") +  
#  scale_fill_manual(values = c("#3A65A3", "#C2DFEB", "#EFA15C", "#BA3832", "black")) +
#  theme(axis.text.x=element_blank(), #remove x axis labels
#        axis.ticks.x=element_blank(), #remove x axis ticks
#        axis.text.y=element_blank(),  #remove y axis labels
...
#  as_tibble() %>%
#  ggplot() + 
# geom_col(aes(ratio_binned_collapsed, perc, fill = ratio_binned_collapsed), colour="black") #+
#  scale_y_continuous(limits = c(0,0.7)) +
#  geom_hline(yintercept = 0)  + 
#  geom_text(aes(ratio_binned_collapsed, perc+0.05, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 3.5, colour = "black") +
#   coord_flip() +
#  scale_fill_manual(values = c("#3A65A3", "#C2DFEB", "#EFA15C", "#BA3832", "black")) +
#  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
#  guides(fill = "none", color = "none") +
...
melbourne_needs_gap_sa1_2021 %>% 
 ...
 geom_col(aes(transit_supply, perc, fill = transit_supply), colour="black") +
  ...
  scale_y_continuous(limits = c(0,0.7)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, perc+0.05, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 2.5) +
  coord_flip() +
   scale_fill_manual(values = c("darkblue", "lightblue", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) + 
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
have been added to the area surrounding the Selandra Rise Estate
between 2016 (Figure \ref{fig:Selandra_rise}(a),
2021 (c)
and 2024 (Figure \ref{fig:Bus_798} b).
This appears to have been sufficient to shift some of the estate 
from the Very Low category in 2016 to Low in 2021. 

Figure \ref{fig:Selandra_rise}(i), however,
indicates that some of the surrounding SA1s,
however,
still fall into the Very High needs and Very Low or Zero supply
needs-gap category (red and dark red),
...
# scale_x_discrete(drop=FALSE) +
  scale_y_continuous(limits = c(0,0.7)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs, perc+0.01, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 2) +
  coord_flip() + 
    scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) + 
 guides(fill = "none", color = "none") +
  labs(
...
inset_element(pcol, left = 0.0, bottom = 0.0, right = 0.4, top =0.30) +
  inset_element(pcol_mornington, left = 0.59, bottom = 0.04, right = 0.89, top =0.19) +
  inset_element(pcol_south_east, left = 0.82, bottom = 0.04, right = 1.12, top =0.19) +
  inset_element(pcol_outer_east, left = 0.87, bottom = 0.60, right = 1.17, top =0.75) +
  inset_element(pcol_north_east, left = 0.59, bottom = 0.81, right = 0.89, top =0.96)  +
  inset_element(pcol_north_west, left = 0.0, bottom = 0.81, right = 0.3, top =0.96) +
  inset_element(pcol_west, left = 0.00, bottom = 0.30, right = 0.3, top =0.45) +
  inset_element(pcol_inner, left = 0.27, bottom = 0.30, right = 0.57, top =0.45) +
  inset_element(pcol_inner_south, left = 0.27, bottom = 0.20, right = 0.57, top =0.35) +  inset_element(pcol_inner_east, left = 0.75, bottom = 0.78, right = 1.05, top =0.93) 
