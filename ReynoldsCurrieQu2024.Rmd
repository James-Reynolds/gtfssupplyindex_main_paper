...
# Introduction
A common role for transit 
in many locations 
is providing mobility 
for those who cannot otherwise drive themselves, 
such that they have some accessibility 
...
These might suggest providing at least some transit, 
...
suggesting an increased need for
transit coverage 
in the future. 
...
(2) to better understand spatial patterns of gaps 
...
Table \ref{tab:Greater_Melbourne_CCDs_SA1_population} compares the share of resident population in each transport supply category. Greater Melbourne's population increased by 
...
(`r label_percent(accuracy=0.1)(sum(melbourne_comparison_2006_2021[1,9])/sum(melbourne_comparison_2006_2021[,9]))`) in 2021. Residents with Zero or Very Low Transport Supply rose by `r label_percent(accuracy=1)(sum(melbourne_comparison_2006_2021[1:2,9])/sum(melbourne_comparison_2006_2021[1:2,3])-1)` from 
...
in 2021. Residents with supply below the average (Zero, Very Low, Low or Below Average) rose by `r label_percent(accuracy=1)(sum(melbourne_comparison_2006_2021[1:4,9])/sum(melbourne_comparison_2006_2021[1:4,3])-1)` from 
...
(`r label_percent(accuracy=0.1)(sum(melbourne_comparison_2006_2021[1,8])/sum(melbourne_comparison_2006_2021[,8]))`)). The number   
Residents with Zero or Very Low Transport Supply rose by 
`r label_percent(accuracy=1)(sum(melbourne_comparison_2006_2021[1:2,9])/sum(melbourne_comparison_2006_2021[1:2,8])-1)` (from 
...
Residents with supply below the average (Zero, Very Low, Low or Below Average) rose `r label_percent(accuracy=1)(sum(melbourne_comparison_2006_2021[1:4,9])/sum(melbourne_comparison_2006_2021[1:4,8])-1)` (from 
...
pcol <- pop_comparison %>% 
  ...
  geom_text(aes(transit_supply, population + 90000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "#5DB000", "#53A212", "#407F0B")) +
...
inset_element(pcol, left = 0.0, bottom = 0.0, right = 0.275, top =0.31) +
...
for areas smaller than Greater Capital City Statistical Areas (GCCSAs) or other agglomerations. The Greater Melbourne GCCSA consists of nine SA4s, all named according to geographic location (including the Mornington Peninsula SA4, which is at the extreme south of the Melbourne GCCSA).]. In 2016,
...
respectively, for 2006 in @currie2010identifying. For 2021 the SI scores average 
 ...
 (+`r label_percent(accuracy=1)(si_CCD2006_21_census_week_aggregated$si_by_area %>% st_drop_geometry() %>% filter(suburbs == "outer") %>% select(SI) %>% unlist() %>% mean()/764.3-1)`) respectively^[The same grouping of Local Government Areas (LGAs) to inner, middle and outer suburb groups 
...
Results found here might not be exactly comparable to those 
in @currie2010identifying due to geographic inconsistencies^[However, the @currie2010identifying results show 5,839 total CCDs within Melbourne, 
...
Overall, however, the results appear to be consistent 
with there having been increases in SI scores 
across all parts of Melbourne since 2006, while 
...
pcol <- si_by_area_2016_2021_wider %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, population + 150000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 2) +
  coord_flip() + 
...
Figure \ref{fig:Greater_Melbourne_2016_2021_ratio_map} shows changes in SI scores between 2016 and 2021. Average SI scores for SA1s across all of Greater Melbourne 
increased from 
`r si_by_area_2016_2021_wider$x2016 %>% na.omit() %>% mean() %>% format(big.mark = ",", digits = 5)` (2016)
 to 
`r si_by_area_2016_2021_wider$x2021 %>% na.omit() %>% mean() %>% format(big.mark = ",", digits = 5)` 
(2021, +`r percent_format(accuracy = 0.1)(si_by_area_2016_2021_wider$x2021 %>% na.omit() %>% mean()/si_by_area_2016_2021_wider$x2016 %>% na.omit() %>%  mean()-1)`). 
...
There is also a statistically significant difference in the proportion of SA1s in each service level change category across SA4 areas 
(`r prmisc::print_chi2(chisq.test(greater_melbourne_ratio_SA1[,2:10] %>% na.omit() %>% as.matrix()))`)^[See Appendix for table]. 
...
and Greater Melbourne as a whole (column 11). In 2021, slightly more than half of the population across Greater Melbourne were living in SA1s where the SI scores were at least one percent higher than they were in 2016 
(`r greater_melbourne_ratio_population_wider[1:6,2:10] %>% sum() %>% format(big.mark = ",")` people, `r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[1:6,2:10] %>% sum() / greater_melbourne_ratio_population_wider[,2:10] %>% sum())`). The share of population for whom the SI increased by at least one percent was highest in the North-West (`r greater_melbourne_ratio_population_wider[1:6,3] %>% sum() %>% format(big.mark = ",")` people, `r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[1:6,3] %>% sum() / greater_melbourne_ratio_population_wider[,3] %>% sum())`) and West (`r greater_melbourne_ratio_population_wider[1:6,6] %>% sum() %>% format(big.mark = ",")` people, `r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[1:6,6] %>% sum() / greater_melbourne_ratio_population_wider[,6] %>% sum())`) SA4s, but lowest in the Inner East 
...
Service levels in 2021 stayed within one percent of the level in 2016 for 
`r greater_melbourne_ratio_population_wider[7,2:10] %>% sum() %>% format(big.mark = ",")` people across Greater Melbourne 
...
The share of population for whom the SI stayed within one percent was highest in the  Outer East 
...
Across all of Greater Melbourne `r greater_melbourne_ratio_population_wider[8:11,2:10] %>% sum() %>% format(big.mark = ",")` people, 
`r label_percent(accuracy = 0.1)(sum(greater_melbourne_ratio_population_wider[8:11,2:10]) /sum(greater_melbourne_ratio_population_wider[,2:10]))`) were living in SA1s 
in 2021 where the SI was more than one percent lower than it had been in 2016.
The share of the population living in areas where the SI had dropped by more than one percent was largest in the Inner
...
## Problem here! Graph does not match table

Across all of Greater Melbourne `r greater_melbourne_ratio_population_wider[11,2:10] %>% sum() %>% format(big.mark = ",")` residents in 2021
(`r label_percent(accuracy = 0.1)(sum(greater_melbourne_ratio_population_wider[11,2:10]) /sum(greater_melbourne_ratio_population_wider[,2:10]))`) were living in SA1s where there had been some service in 2016, but this had been withdrawn. The share of the population living where transit service had been withdrawn was largest in the Mornington Peninsula (`r label_percent(accuracy = 0.1)(sum(greater_melbourne_ratio_population_wider[11,"Mornington Peninsula"]) /sum(greater_melbourne_ratio_population_wider[,"Mornington Peninsula"]))`, `r greater_melbourne_ratio_population_wider[12,"Mornington Peninsula"] %>% sum() %>% format(big.mark = ",")`) and
South East (`r label_percent(accuracy = 0.1)(sum(greater_melbourne_ratio_population_wider[12,"South East"]) /sum(greater_melbourne_ratio_population_wider[,"South East"]))`, `r greater_melbourne_ratio_population_wider[12,"South East"] %>% sum() %>% format(big.mark = ",")`) SA4s, whereas no one at all in the Inner SA4 was living in a SA1 without at least a Very Low service level. 
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne_sa4 %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs, population + 150000, label = glue::glue("{label_percent(accuracy = 0.1)(perc)}")), size = 2) +
  coord_flip() + 
    scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) + 
 guides(fill = "none", color = "none") +
  labs(
...
pcol_mornington <- combined_needs_index_2021_sa1_greater_melbourne_sa4 %>% 
  ...
  geom_col(aes(composite_needs, population, fill = composite_needs)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs, population + 5000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_north_west <- combined_needs_index_2021_sa1_greater_melbourne_sa4 %>% 
  ...
  geom_col(aes(composite_needs, population, fill = composite_needs)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs, population + 9000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_south_east <- combined_needs_index_2021_sa1_greater_melbourne_sa4 %>% 
  ...
  geom_col(aes(composite_needs, population, fill = composite_needs)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs, population + 13000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
