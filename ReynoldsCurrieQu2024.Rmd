---
title: "Transit access, frequency and gaps related to vertical social equity: new GTFS tools"
author:
  - name: James Reynolds
...
```{r Currie_map_SI, out.width = "90%", fig.cap = "Melbourne 2006: Distribution of supply measure scores (top), needs (middle) and gaps (bottom), Source: Currie (2010)",  echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE}

knitr::include_graphics("graphics/Currie2010combined1.png")
...
Table \ref{tab:Greater_Melbourne_CCDs_SA1_table} summarises the distribution of CCDs and SA1s across different Transport Supply categories in 2006, 2016 and 2021. Figure \ref{fig:Greater_Melbourne_2021_SA1_plot} shows the spatial distribution of Transport Supply in 2021. Maps for 2016 and CCDs are included in the Appendix. 
...
north_east_sa4 <- sa42021 %>% filter(sa4_name_2021 == "Melbourne - North East")
ptv_210805_gtfs_north_east <- tidytransit::filter_feed_by_area(ptv_210805_gtfs, north_east_sa4)

#convert to sf
ptv_210805_gtfs_north_east <- tidytransit::gtfs_as_sf(ptv_210805_gtfs_north_east)
ptv_210805_gtfs_north_east <- tidytransit::get_route_geometry(ptv_210805_gtfs_north_east)

#ggplot() + 
#  geom_sf(data=ptv_210805_gtfs_north_east) +     
#  coord_sf(ylim = c(-37.50, -37.70), 
#           xlim = c(144.85, 145.20))


north_west_sa4 <- sa42021 %>% filter(sa4_name_2021 == "Melbourne - North West")
ptv_210805_gtfs_north_west <- tidytransit::filter_feed_by_area(ptv_210805_gtfs, north_west_sa4)
...
## filter feed to only include North East SA4s
ptv_160804_gtfs_north_east <- tidytransit::filter_feed_by_area(ptv_160804_gtfs, north_east_sa4)

#convert to sf
ptv_160804_gtfs_north_east <- tidytransit::gtfs_as_sf(ptv_160804_gtfs_north_east)
ptv_160804_gtfs_north_east <- tidytransit::get_route_geometry(ptv_160804_gtfs_north_east)

```

```{r north_east_changes, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.dim = c(7.5,7.5), fig.cap="Part of the North East SA4 region (centred on Epping North): Transport Supply categories in 2016 (top left) and 2021 (top right), change in SI score between 2016 and 2021 for SA1s with Very High needs but Very Low or Zero supply with population in 2016 (bottom left) and 2021 (bottom right), overlayed with transit routes (purple). "}




north_east_very_high_needs_SI_change <- ggplot() + 
  geom_sf(data=si_by_area_2016_2021_wider %>% 
            na.omit() %>%
            filter(very_high_needs_zero_or_very_low_supply == TRUE),
          aes(fill = ratio_binned), colour=NA) + 
...
#Plot 
melbourne_si_SA12016_16_plot + 
  geom_sf(data=ptv_160804_gtfs_north_east, colour="purple") +
  coord_sf(ylim = c(-37.60, -37.65), 
           xlim = c(144.98, 145.05)) 

melbourne_si_SA12021_21_plot +
  geom_sf(data=ptv_210805_gtfs_north_east, colour="purple") +
  coord_sf(ylim = c(-37.60, -37.65), 
           xlim = c(144.98, 145.05))  +
  ggrepel::geom_label_repel(
    data = si_by_area_2016_2021_wider %>% 
...
north_east_very_high_needs_SI_change + 
  geom_sf(data=ptv_210805_gtfs_north_east, colour="purple") +
  coord_sf(ylim = c(-37.60, -37.65), 
           xlim = c(144.98, 145.05)) +
  ggrepel::geom_label_repel(
...
melbourne_si_SA12016_16_plot + 
  ...
  coord_sf(ylim = c(-38.08, -38.13), 
           xlim = c(145.27, 145.35)) +
  melbourne_si_SA12021_21_plot +
