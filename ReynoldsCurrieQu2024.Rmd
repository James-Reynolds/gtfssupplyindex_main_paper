...
pcol <- pop_comparison %>% 
  ...
  geom_text(aes(transit_supply, population + 100000, label = glue::glue("{label_percent(accuracy = 0.1)(perc)}"))) +
  coord_flip() +
  scale_fill_manual(values = c("black", "#F9D8B1", "#F7C387", "#F29C33", "#5DB000", "#53A212", "#407F0B")) +
...
Greater_Melbourne_2016_2021_ratio_map <- ggplot()+ 
  geom_sf(data=si_by_area_2016_2021_wider %>% 
            na.omit() %>%
            filter(ratio_binned != "Never served"),
          aes(fill = ratio_binned), colour=NA) +  scale_fill_manual(values = c("magenta", "#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "black")) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
        axis.ticks.y=element_blank(), #remove y axis ticks
...
pcol <- si_by_area_2016_2021_wider %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, population + 100000, label = glue::glue("{label_percent(accuracy = 0.1)(perc)}"))) +
  coord_flip() + 
  scale_fill_manual(values = c("magenta", "#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "black"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "Population by Transit Supply",
...
scale_fill_manual(values = c("magenta", "#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "black")) +
  ...
  ggrepel::geom_label_repel(
    data = si_by_area_2016_2021_wider %>% 
      na.omit() %>% 
      filter(very_high_needs_zero_or_very_low_supply == TRUE) %>% 
      filter(ratio_binned == "Service withdrawn"),
