...
melbourne_si_SA12021_21_plot + 
  ggspatial::annotation_scale(location = 'br') + 
  ggspatial::annotation_north_arrow(location = 'tr') +
inset_element(pcol, left = 0.0, bottom = 0.0, right = 0.40, top =0.31) +
  inset_element(pcol_mornington, left = 0.56, bottom = 0.05, right = 0.86, top =0.2) +
  inset_element(pcol_south_east, left = 0.82, bottom = 0.05, right = 1.12, top =0.2) +
  inset_element(pcol_outer_east, left = 0.75, bottom = 0.686, right = 1.0, top =0.83) +
  inset_element(pcol_north_east, left = 0.58, bottom = 0.80, right = 0.83, top =0.95)  +
  ...
  inset_element(pcol_inner, left = 0.2, bottom = 0.31, right = 0.5, top =0.44) +
  inset_element(pcol_inner_south, left = 0.34, bottom = 0.25, right = 0.64, top =0.38) +
...
```{r Greater_Melbourne_2016_2021_ratio_map, fig.show="hold", fig.dim=c(8,8),  echo = FALSE, warning=FALSE, message=FALSE, cache = FALSE, include = TRUE, fig.cap="Greater Melbourne: changes in SI between 2016 and 2021, by 2021 SA1, overlayed with 2006 Greater Melbourne boundary (black); 2021 SA4 boundaries (blue); and suburban railway lines (black)"}
...
Greater_Melbourne_2016_2021_ratio_map +
  ggspatial::annotation_scale(location = 'br') + 
  ggspatial::annotation_north_arrow(location = 'tr') + 
  inset_element(pcol, left = 0.0, bottom = 0.0, right = 0.45, top =0.3) +
  ...
  inset_element(pcol_outer_east, left = 0.7, bottom = 0.84, right = 1.0, top =0.99) +
  inset_element(pcol_north_east, left = 0.53, bottom = 0.85, right = 0.83, top =1.0)  +
  inset_element(pcol_north_west, left = 0.1, bottom = 0.80, right = 0.4, top =0.95) +
  inset_element(pcol_west, left = 0.0, bottom = 0.5, right = 0.3, top =0.65) +
  inset_element(pcol_inner, left = 0.32, bottom = 0.37, right = 0.62, top =0.49) +
  inset_element(pcol_inner_south, left = 0.351, bottom = 0.22, right = 0.651, top =0.37) +
...
melbourne_si_SA12021_21_plot +
  ggspatial::annotation_scale(location = 'br') + 
  ggspatial::annotation_north_arrow(location = 'tr') + 
  inset_element(pcol, left = 0.0, bottom = 0.0, right = 0.4, top =0.25) +
  inset_element(pcol_mornington, left = 0.48, bottom = 0.0, right = 0.78, top =0.12) +
  ...
  inset_element(pcol_north_west, left = 0.02, bottom = 0.81, right = 0.32, top =0.96) +
  inset_element(pcol_west, left = 0.0, bottom = 0.45, right = 0.3, top =0.60) +
  inset_element(pcol_inner, left = 0.30, bottom = 0.30, right = 0.60, top =0.45) +
  inset_element(pcol_inner_south, left = 0.30, bottom = 0.15, right = 0.60, top =0.30) +
  inset_element(pcol_inner_east, left = 0.56, bottom = 0.56, right = 0.85, top =0.69) 
...
```{r Greater_Melbourne_2021_needs_gap_scatterplot, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE,  fig.dim=c(8,8), fig.cap="Greater Melbourne 2021, SI and Combined Needs Index scores. SI scores < 10 rounded up to equal 10 to improve legibility."}
...
```{r Greater_Melbourne_2021_needs_gap_map_figure, eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.dim=c(8,8),  fig.cap="Greater Melbourne 2021: Very High needs SA1s (only) Transport Supply groupings,"}
...
```{r Greater_Melbourne_2016_needs_gap_SA4_service_change, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.dim=c(8,8), fig.cap="Greater Melbourne: change in transit supply between 2016 and 2021 for SA1s with Very High needs but Very Low or Zero supply in 2016"}
...
pcol <- melbourne_si_SA12016_16_census_week_aggregated$si_by_area %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, population + 2000, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 3.5) +
  coord_flip() + 
 scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "#5DB000", "#53A212", "#407F0B")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
#        legend.key.size = unit(0.08, 'cm')
  ...
  inset_element(pcol, left = 0.6, bottom = 0.6, right = 1, top = 1)
...
pcol <- melbourne_needs_gap_sa1_2021 %>% 
          st_drop_geometry() %>%
          filter(composite_needs == "Very High") %>%
  filter(sa2_name_2021 == "Cranbourne" | sa2_name_2021 == "Cranbourne East - North") %>%
  na.omit() %>%
 ...
 geom_col(aes(transit_supply, perc, fill = transit_supply)) +
  scale_y_continuous(limits = c(0,0.7)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, perc+0.1, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 3) +
  coord_flip() +
  scale_fill_manual(values = c("darkred", "red", "#F7C387", "#F29C33", "#5DB000", "#53A212", "#407F0B")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
#        legend.key.size = unit(0.08, 'cm')
  ...
  geom_sf(data=melbourne_railway_trips_shape_2021, linetype = "twodash", size = 0.1) + 
    coord_sf(ylim = c(-38.082, -38.128), 
           xlim = c(145.25, 145.360)) +
  inset_element(pcol, left = 0.65, bottom = 0.0, right = 0.9, top = 0.3)
...
ggplot()+ 
  geom_sf(data=si_by_area_2016_2021_wider %>% 
            left_join(melbourne_needs_gap_sa1_2021 %>% st_drop_geometry() %>% select(sa1_code_2021, very_high_needs_very_low_zero_supply_2021)) %>%
