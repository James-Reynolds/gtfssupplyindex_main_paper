...
A role for transit 
in many locations 
is to provide at least a basic level of mobility 
so that those who cannot otherwise drive themselves 
have some accessibility 
to activities and services 
beyond walking distance [@Currie:2016aa].
Age, 
disability, 
...
As well, lower proportions of younger people are obtaining a 
driving license than was the case previously [@delbosc2013causes], 
suggesting the community's need 
for transit coverage 
may be likely to increase into the future. 
...
the identified geographic patterns 
...
It also facilitates data access and processing for research purposes. 
However, tools
for using GTFS data 
to examine spatial 
patterns and gaps 
...
Ojectives of this research are: 
(1) to develop tools for undertaking needs-gap analysis 
using GTFS datasets; and 
(2) to understand spatial patterns of gaps 
between social needs for transport and transit supply, 
including whether those reported in @Currie2007Identifying 
and @currie2010identifying 
are consistent with the current situation. 
...
for comparison to the 2006 results 
reported in @currie2010identifying^[The 
wider programme of research 
includes examination of spatial gaps in other cities, 
so as to better understand whether patterns in Melbourne 
are representative of other places. 
However, this paper is limited to examining Melbourne 
in 2016 and 2021 only. 
Results for other cities will be reported elsewhere.].
...
and limitations of this study and directions for future research are discussed in Section 6, which concludes the paper. 

# Research context
There are 
many metrics 
available for assessing transit services. 
Examples include: those in 
the *Transit Cooperative Research Program (TCRP) Report 88: 
a guidebook for developing performance-measurement systems* [@Ryus:2003aa]; 
and those used across benchmarking databases and programs 
such as by @Florida-Transit-Information-System:2018aa,  @UITP:2015aa and @Imperial-College-London:2023aa.
...
or assess proposed system changes. 
Contrasting examples are provided by:

- *Transit Scores* [@WalkScore:2023tg],  
...
for measuring many different aspects of a transit system. 
Scores are consistently grouped into five categories, 
providing a simple ranking from very good to not very good, 
and can be independently calculated (with sufficient data).
However, there are many metrics,  
which may complexity to using TCQSM scores
for communication with non-technical audiences 
(e.g. the public, politicians or other stakeholders).

GTFS datasets have allowed the application of such metrics 
to many transit systems with, for example, 
the *Transit Score* website reporting scores 
across the world. 
@Wong:2013aa provides another example, 
in reporting the distribution of
...
The lack of a similar code base 
...
reproduced in Figure \ref{fig:Currie_map_SI}(b), 
...
and in some middle suburbs in the south-east,
north and west.

@currie2010identifying compared social needs and Supply Index scores (Figure \ref{fig:Currie_map_SI}(c)) and identified 
areas with very high transport needs, 
but very low or zero transit supply (Figure \ref{fig:Currie_map_SI}(d)), where service gaps 
might be of particular concern. 
Most of these were located in the outer suburbs of Melbourne 
...
The case-based methods used in this research to develop such tools are discussed in the following.
...
but researchers lack control of events 
(preventing experiments)[@Yin2009aa]. 
Here research questions relate to: 
(1) how to use GTFS data 
to assess gaps between social needs 
and transit supply, and 
(2) how and why spatial patterns might have changed 
...
being the need to seek generalisable findings 
while at the same time being grounded 
in the context of only a small number of cases 
(allowing the study to go into greater depth)[@Denscombe2007aa; @Ketokivi2014aa]. 
...
Here the duality criterion is addressed by 
developing software functions are generic 
and might be applied to other GTFS feeds, 
...
ABS data was used to define areas of interest, 
and was sourced via the absmapsdata package [@R-absmapsdata].


## Melbourne Case Study

The methodological literature provides guidance on case selection
and discusses various theoretical sampling approaches, 
including sampling for critical, 
particularly revelatory and/or 
representative cases [@Eisenhardt1989aa; @Yin2009aa; @Denscombe2007aa; @Eisenhardt2007TBfC]. 
@Yin2009aa notes selection of a case to allow longitudinal study,
and this is the primary reason for selecting Melbourne here to facilitate comparison with the 2006 results reported in @currie2010identifying.
As such, SI scores were calculated 
using the same 
...
Statistical Area (SA1) zones, 
under a hierarchical structure ranging from SA1s (~400 people) 
to SA4s (parts of a city or region). 
As such, SI scores have also been calculated 
...
changes to ABS reporting means
that the composite needs indicator used by @currie2010identifying
cannot be exactly replicated. 
Hence, the approach used here 
includes only two components 
in the composite needs index^[In contrast to the four 
of @currie2010identifying, 
which included two "relative need" components 
obtained by weighting the IRSAD and the transport needs indexes
by the population within the various needs groups 
for each area of interest. 
Current ABS reporting, 
however, does not allow the total number of people within 
one or more of the various needs groups to be identified 
at the SA1 level.] 
based on weighting 
both the IRSAD index 
and the transport need index 
by the total population of each SA1. 
These were then standardised and grouped, 
as per the six groups used by @currie2010identifying^[Very Low, Low, Below average, Above average, High and Very High.].
...
Table \ref{tab:Greater_Melbourne_CCDs_SA1_table} 
summarises the distribution of CCDs and SA1s 
across different Transport Supply categories 
in 2006, 2016 and 2021. 
Figures \ref{fig:Greater_Melbourne_population_2016_by_SA4} and 
\ref{fig:Greater_Melbourne_population_2021_by_SA4}
shows the spatial distribution of 
Transport Supply by SA1 in 2016 and 2021. 
Additional maps showing Transport Supply by CCD are included in the Appendix. 
...
Residents in areas with Zero Supply rose from `r format(sum(melbourne_comparison_2006_2021[1,3]), big.mark = ",")` 
...
(`r label_percent(accuracy=0.1)(sum(melbourne_comparison_2006_2021[1,9])/sum(melbourne_comparison_2006_2021[,9]))`) in 2021, representing a 
`r label_percent(accuracy=1)(sum(melbourne_comparison_2006_2021[1,9])/sum(melbourne_comparison_2006_2021[1,3])-1)` 
increase. This compares to the 
...
Residents with Zero or Very Low Transport Supply rose from 
...
Residents with supply below the average (Zero, Very Low, Low or Below Average) rose from 
...
(`r label_percent(accuracy=0.1)(sum(melbourne_comparison_2006_2021[1:4,9])/sum(melbourne_comparison_2006_2021[,9]))`) in 2021, representing a
`r label_percent(accuracy=1)(sum(melbourne_comparison_2006_2021[1:4,9])/sum(melbourne_comparison_2006_2021[1:4,3])-1)` increase (
...
pcol <- pop_comparison %>% 
  ...
  geom_text(aes(transit_supply, population + 80000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "#5DB000", "#53A212", "#407F0B")) +
...
pcol_mornington <- melbourne_2016_population_transit_supply_by_SA4 %>% 
  ...
  geom_col(aes(transit_supply, population, fill = transit_supply)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, population + 20000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "#5DB000", "#53A212", "#407F0B")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_inner_east <- melbourne_2016_population_transit_supply_by_SA4 %>% 
  ...
  geom_col(aes(transit_supply, population, fill = transit_supply)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, population + 15000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "#5DB000", "#53A212", "#407F0B")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_north_east <- melbourne_2016_population_transit_supply_by_SA4 %>% 
  ...
  geom_col(aes(transit_supply, population, fill = transit_supply)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, population + 10000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "#5DB000", "#53A212", "#407F0B")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_south_east <- melbourne_2016_population_transit_supply_by_SA4 %>% 
  ...
  geom_col(aes(transit_supply, population, fill = transit_supply)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, population + 18000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "#5DB000", "#53A212", "#407F0B")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_inner <- melbourne_2016_population_transit_supply_by_SA4 %>% 
  ...
  geom_col(aes(transit_supply, population, fill = transit_supply)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, population + 60000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "#5DB000", "#53A212", "#407F0B")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
melbourne_si_SA12016_16_plot + 
inset_element(pcol, left = 0.0, bottom = 0.0, right = 0.25, top =0.3) +
...
pcol_inner_south <- melbourne_2021_population_transit_supply_by_SA4 %>% 
  ...
  geom_col(aes(transit_supply, population, fill = transit_supply)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, population + 25000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "#5DB000", "#53A212", "#407F0B")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol <- si_by_area_2016_2021_wider %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, population + 90000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 2) +
  coord_flip() + 
...
pcol_mornington <- si_by_area_2016_2021_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "Mornington Peninsula") %>%
  aggregate(population ~ ratio_binned, sum) %>%
  mutate(perc=population/sum(population)) %>% 
  ggplot() + 
  geom_col(aes(ratio_binned, population, fill = ratio_binned)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, population + 9000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 2) +
  coord_flip() +
  scale_fill_manual(values = c( "#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "magenta", "black"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_inner_east <- si_by_area_2016_2021_wider %>%   
  st_drop_geometry() %>% 
  filter(sa4_name_2021 == "Inner East") %>%
  aggregate(population ~ ratio_binned, sum) %>%
  mutate(perc=population/sum(population)) %>% 
  ...
  geom_col(aes(ratio_binned, population, fill = ratio_binned)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, population + 20000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 2) +
  coord_flip() +
  scale_fill_manual(values = c( "#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "magenta", "black"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_inner_south <- si_by_area_2016_2021_wider %>%   
  st_drop_geometry() %>% 
  filter(sa4_name_2021 == "Inner South") %>%
  aggregate(population ~ ratio_binned, sum) %>%
  mutate(perc=population/sum(population)) %>% 
  ...
  geom_col(aes(ratio_binned, population, fill = ratio_binned)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, population + 12000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 2) +
  coord_flip() +
  scale_fill_manual(values = c( "#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "magenta", "black"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_north_east <- si_by_area_2016_2021_wider %>%   
  st_drop_geometry() %>% 
  filter(sa4_name_2021 == "North East") %>%
  aggregate(population ~ ratio_binned, sum) %>%
  mutate(perc=population/sum(population)) %>% 
  ...
  geom_col(aes(ratio_binned, population, fill = ratio_binned)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, population + 8000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 2) +
  coord_flip() +
  scale_fill_manual(values = c( "#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "magenta", "black"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_north_west <- si_by_area_2016_2021_wider %>%   
  st_drop_geometry() %>% 
  filter(sa4_name_2021 == "North West") %>%
  aggregate(population ~ ratio_binned, sum) %>%
  mutate(perc=population/sum(population)) %>% 
  ...
  geom_col(aes(ratio_binned, population, fill = ratio_binned)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, population + 6000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 2) +
  coord_flip() +
  scale_fill_manual(values = c( "#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "magenta", "black"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_outer_east <- si_by_area_2016_2021_wider %>%   
  st_drop_geometry() %>% 
  filter(sa4_name_2021 == "Outer East") %>%
  aggregate(population ~ ratio_binned, sum) %>%
  mutate(perc=population/sum(population)) %>% 
...
pcol_south_east <- si_by_area_2016_2021_wider %>%   
  st_drop_geometry() %>% 
  filter(sa4_name_2021 == "South East") %>%
  aggregate(population ~ ratio_binned, sum) %>%
  mutate(perc=population/sum(population)) %>% 
...
pcol_west <- si_by_area_2016_2021_wider %>%   
  st_drop_geometry() %>% 
  filter(sa4_name_2021 == "West") %>%
  aggregate(population ~ ratio_binned, sum) %>%
  mutate(perc=population/sum(population)) %>% 
...
pcol_inner <- si_by_area_2016_2021_wider %>%   
  st_drop_geometry() %>% 
  filter(sa4_name_2021 == "Inner") %>%
  aggregate(population ~ ratio_binned, sum) %>%
  mutate(perc=population/sum(population)) %>% 
...
Greater_Melbourne_2016_2021_ratio_map + 
  inset_element(pcol, left = 0.0, bottom = 0.0, right = 0.3, top =0.25) +
  inset_element(pcol_mornington, left = 0.58, bottom = 0.03, right = 0.78, top =0.18) +
  inset_element(pcol_south_east, left = 0.8, bottom = 0.05, right = 1, top =0.20) +
  inset_element(pcol_outer_east, left = 0.78, bottom = 0.7, right = 1.0, top =0.85) +
  inset_element(pcol_north_east, left = 0.58, bottom = 0.85, right = 0.83, top =1.0)  +
  inset_element(pcol_north_west, left = 0.0, bottom = 0.85, right = 0.23, top =1) +
  inset_element(pcol_west, left = 0.03, bottom = 0.305, right = 0.28, top =0.455) +
...
pcol_mornington <- combined_needs_index_2021_sa1_greater_melbourne_sa4 %>% 
  ...
  geom_col(aes(composite_needs, population, fill = composite_needs)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs, population + 10000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_inner_east <- combined_needs_index_2021_sa1_greater_melbourne_sa4 %>% 
  ...
  geom_col(aes(composite_needs, population, fill = composite_needs)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs, population + 15000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_inner_south <- combined_needs_index_2021_sa1_greater_melbourne_sa4 %>% 
  ...
  geom_col(aes(composite_needs, population, fill = composite_needs)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs, population + 25000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_south_east <- combined_needs_index_2021_sa1_greater_melbourne_sa4 %>% 
  ...
  geom_col(aes(composite_needs, population, fill = composite_needs)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs, population + 18000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_west <- combined_needs_index_2021_sa1_greater_melbourne_sa4 %>% 
  ...
  geom_col(aes(composite_needs, population, fill = composite_needs)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs, population + 20000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_inner <- combined_needs_index_2021_sa1_greater_melbourne_sa4 %>% 
  ...
  geom_col(aes(composite_needs, population, fill = composite_needs)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs, population + 60000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 1.8) +
  coord_flip() +
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
Figures \ref{fig:Greater_Melbourne_2016_needs_gap_map_figure} and \ref{fig:Greater_Melbourne_2021_needs_gap_map_figure} show SA1 zones in Greater Melbourne with Very High transport needs, but Very Low or Zero Transport Supply for 2016 and 2021. Comparison to the 2006 spatial distribution shown in Figure \ref{fig:Currie_map_SI} (bottom) suggests that areas with large gaps between social needs and Transport Supply continue to be mostly in the outer areas of Melbourne. However, such comparisons appear complicated by the larger number of SA1s than CCDs, with Figures \ref{fig:Greater_Melbourne_2016_needs_gap_map_figure} and \ref{fig:Greater_Melbourne_2021_needs_gap_map_figure} appearing to show more (smaller) areas with Very High transport needs, but Very Low or Zero Transport Supply. This includes areas in the West, North West, North East and South East SA4s, whereas in 2006 these parts of Melbourne did not appear to have large gaps between need and supply. The bottom parts of the Morning Peninsula (around Blairgowrie) drop out of the category of having the largest needs-gaps in 2016, but are back in this category again in 2021. 
...
```{r map_routes, eval = FALSE, echo = FALSE, warning=FALSE, message=FALSE}
...
Figure \ref{fig:Selandra_rise} revisits the Selandra Rise Estate, showing Transport Supply in 2016 and 2021, and changes in SI score for SA1s with Very High social needs, but Very Low or Zero Transport Supply.  
...
```{r Greater_Melbourne_SA12016_plot_appendix, fig.show="hold", echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.dim = c(7.5,7.5), fig.cap="Greater Melbourne, Transport Supply by SA1 for the week starting the date of the 2016 census, overlayed with: 2006 Greater Melbourne boundary (black); 2021 SA4 boundaries (blue); and suburban railway lines (black)"}
