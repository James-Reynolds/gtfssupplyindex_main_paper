...
Along these lines @Currie2003Hobart, 
...
the spatial patterns identified for Melbourne in 2006
have changed in the intervening years 
or are representatvie of other places.
...
cleaning and analysis of data for each operator. 
Nowadays, however,
...
and @currie2010identifying 
are representative of the current situation 
...
- *Transit Score* [@WalkScore:2023tg], 
...
for many different aspects of a transit system. 
...
the public, politicians or other stakeholders.

GTFS datasets have allowed the application of such metrics 
to many transit systems. 
The *Transit Score* website provides an example, 
...
This approach does not appear to have
been adopted widely 
...
so that the duality criterion is addressed by 
developing generic software functions 
...
There are, 
however, 
limitations to the generalisability 
of this research with respect to changes since 2006 
...
and while  @Currie2003Hobart and @Currie2004Gap reported results for Hobart and Adelaide 
these used earlier versions of the needs-gap assessment methodology, 
for which software tools have not been developed here. 
While this study does seek to findings 
about changes in spatial patterns of social needs and transit supply 
that are generalisable to more than just Melbourne, 
a lack of 2006 results for other cities may limit the extent to which these findings might be confidently considered representative of changes in other places^[Other parts of this research programme will examine changes over the last 10-15 years in other cities (where GTFS data is available). The focus of this paper, however, is on Melbourne.].
...
so as to facilitate comparison with the 2006 results reported in @currie2010identifying.
...
Additional maps showing Transport Supply by CCD and for 2016 are included in the Appendix. 
...
(`r label_percent(accuracy=0.1)(sum(melbourne_comparison_2006_2021[1,3])/sum(melbourne_comparison_2006_2021[,3]))` of the total population) in 2006 to 
...
in 2021, while those with supply below the average (Zero, Very Low, Low or Below Average) rose by `r label_percent(accuracy=1)(sum(melbourne_comparison_2006_2021[1:4,9])/sum(melbourne_comparison_2006_2021[1:4,3])-1)` from 
...
(`r label_percent(accuracy=0.1)(sum(melbourne_comparison_2006_2021[1,8])/sum(melbourne_comparison_2006_2021[,8]))`)). The number of residents with Zero or Very Low Transport Supply rose by 
...
(`r label_percent(accuracy=0.1)(sum(melbourne_comparison_2006_2021[1:2,8])/sum(melbourne_comparison_2006_2021[,8]))`)), while 
...
```
 ...
 of 2,886.9 across Melbourne, with averages of 10,922.7, 2,694.9 and 764.3 across the inner, middle and outer suburbs, 
respectively. In comparison, the average SI value found here for 2021 (using CCDs and within the 2006 boundary) is 
 `r si_CCD2006_21_census_week_aggregated[["si_by_area"]] %>% select(SI) %>% st_drop_geometry() %>% unlist()%>% mean() %>% format(big.mark = ",", digits = 5)`, and for the inner, middle and outer suburbs 
...
in @currie2010identifying due to geographic inconsistencies^[The @currie2010identifying results show 5,839 total CCDs within Melbourne, 
...
Average SI scores for SA1s across all of Greater Melbourne 
increased from 
...
Figure \ref{fig:Greater_Melbourne_2016_2021_ratio_map} shows changes in SI scores between 2016 and 2021, categorised into those SA1s where service levels have reduces (3 groups), stayed within 1% (1 group), increased (5 groups), and where transit services have been introduced, were withdrawn or were not provided in either 2016 or 2021. 
...
(`r prmisc::print_chi2(chisq.test(greater_melbourne_ratio_SA1[c(1:10,12),2:10] %>% na.omit() %>% as.matrix()))`)^[Service withdrawn category removed due to low numbers. See Appendix for table]. 

Table \ref{tab:Greater_Melbourne_2016_2021_ratio_map} 
shows changes in SI score between 2016 and 2021 (column 1) by 
the 2021 population, summarised for each SA4 zone (columns 2 to 10) 
...
(`r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[1:6,2:10] %>% sum() / greater_melbourne_ratio_population_wider[,2:10] %>% sum())`, `r greater_melbourne_ratio_population_wider[1:6,2:10] %>% sum() %>% format(big.mark = ",")` people). The share of population for whom the SI increased by at least one percent was highest in the North-West (`r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[1:6,3] %>% sum() / greater_melbourne_ratio_population_wider[,3] %>% sum())`, `r greater_melbourne_ratio_population_wider[1:6,3] %>% sum() %>% format(big.mark = ",")` people) and West (`r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[1:6,6] %>% sum() / greater_melbourne_ratio_population_wider[,6] %>% sum())`, `r greater_melbourne_ratio_population_wider[1:6,6] %>% sum() %>% format(big.mark = ",")` people) SA4s, but lowest in the Inner East 
(`r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[1:6,9] %>% sum() / greater_melbourne_ratio_population_wider[,9] %>% sum())`, `r greater_melbourne_ratio_population_wider[1:6,9] %>% sum() %>% format(big.mark = ",")` people) 
and Outer East (`r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[1:6,4] %>% sum() / greater_melbourne_ratio_population_wider[,4] %>% sum())`, `r greater_melbourne_ratio_population_wider[1:6,4] %>% sum() %>% format(big.mark = ",")` people) SA4s.

Service levels stayed within one percent of the level in 2016 for 
`r greater_melbourne_ratio_population_wider[7,2:10] %>% sum() %>% format(big.mark = ",")` people living in Greater Melbourne in 2021
...
(`r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[7,4] %>% sum() /sum(greater_melbourne_ratio_population_wider[,4]))`, `r greater_melbourne_ratio_population_wider[7,4] %>% sum() %>% format(big.mark = ",")` people) 
and Inner East 
(`r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[7,9] %>% as.numeric() /sum(greater_melbourne_ratio_population_wider[,9]))`, `r greater_melbourne_ratio_population_wider[7,9] %>% sum() %>% format(big.mark = ",")` people).
...
(`r label_percent(accuracy = 0.1)(sum(greater_melbourne_ratio_population_wider[8:11,8])/sum(greater_melbourne_ratio_population_wider[,8]))`, `r greater_melbourne_ratio_population_wider[8:11,8] %>% sum() %>% format(big.mark = ",")` people), 
Inner South (`r label_percent(accuracy = 0.1)(sum(greater_melbourne_ratio_population_wider[8:11,10]) /sum(greater_melbourne_ratio_population_wider[,10]))`, `r greater_melbourne_ratio_population_wider[8:11,10] %>% sum() %>% format(big.mark = ",")` people) 
and Outer East 
(`r label_percent(accuracy = 0.1)(sum(greater_melbourne_ratio_population_wider[8:11,4]) /sum(greater_melbourne_ratio_population_wider[,4]))`, `r greater_melbourne_ratio_population_wider[8:11,4] %>% sum() %>% format(big.mark = ",")` people) SA4s. 


Across all of Greater Melbourne `r greater_melbourne_ratio_population_wider[12,2:10] %>% sum() %>% format(big.mark = ",")` people
...
Figure \ref{fig:Greater_Melbourne_2021_needs_gap_scatterplot_figure} shows social needs and SI scores for 2021^[To improve the clarity of the figure, SI scores less than 10 have been adjusted to equal 10. As well, those SA1s with combinations of Zero or Very Low Transport Supply and (especially) high needs scores (more than 40); or Very High supply and Very Low needs scores (less than 4.5) have been labelled with their SA2 name, so as to given an indication of which suburbs of Melbourne are at each of the extremes.]. This is analogous to the 2006 plot shown in Figure 1(c). There is a significant, but only weakly positive correlation between the SI and Combined Needs Index scores for both 2016 (`r prmisc::print_cortest(cor.test(melbourne_needs_gap_sa1_2021$combined_needs_index, melbourne_needs_gap_sa1_2021$SI, method = "pearson"))`, `r prmisc::print_cortest(cor.test(melbourne_needs_gap_sa1_2021$combined_needs_index, melbourne_needs_gap_sa1_2021$SI, method = "spearman", exact = FALSE))`) and 2021 (`r prmisc::print_cortest(cor.test(melbourne_needs_gap_sa1_2021$combined_needs_index, melbourne_needs_gap_sa1_2021$SI, method = "pearson"))`, `r prmisc::print_cortest(cor.test(melbourne_needs_gap_sa1_2021$combined_needs_index, melbourne_needs_gap_sa1_2021$SI, method = "spearman", exact = FALSE))`). 
...
in 2016 (`r prmisc::print_chi2(melbourne_sa1_need_gap_crosstable_2016[,2:7] %>% as.matrix())` 
and 2021 (`r prmisc::print_chi2(melbourne_sa1_need_gap_crosstable_2021[,2:7] %>% as.matrix())`)^[See Appendix 
for tables by SA1.]. 
Tables \ref{tab:Greater_Melbourne_2016_needs_gap_population} and \ref{tab:Greater_Melbourne_2021_needs_gap_population} compare
the populations within each Transport Supply and Combined Needs Index grouping for 2016 and 2021. 
In 2016 `r format(melbourne_sa1_need_gap_population_crosstable_2016 %>% filter(transit_supply %in% c("Zero Supply","Very Low")) %>% filter(composite_needs == "Very High") %>% select(population) %>% sum(), big.mark = ",")` 
people lived within SA1s that have Zero or Very Low Transport Supply, 
...
By 2021 this had increased to  `r format(melbourne_sa1_need_gap_population_crosstable_2021 %>% filter(transit_supply %in% c("Zero Supply","Very Low")) %>% filter(composite_needs == "Very High") %>% select(population) %>% sum(), big.mark = ",")` people 
...
si_by_areaSA12016_2016_2021_wider$ratio_binned_collapsed <- factor(
  if_else(si_by_areaSA12016_2016_2021_wider$ratio_binned == "New service" | 
           si_by_areaSA12016_2016_2021_wider$ratio_binned == "Increased 30% or more", "New service or increase of 30% or more", 
  ...
  levels = 
    c("New service or increase of 30% or more", 
      "Increased 1 to 30%", 
      "Within 1%",
      "Service reduced or withdrawn",
      "Never served")
...
si_by_areaSA12016_2016_2021_wider_table_sa1 <- si_by_areaSA12016_2016_2021_wider %>% 
  st_drop_geometry() %>%
  na.omit() %>% 
  filter(very_high_needs_zero_or_very_low_supply == TRUE) %>% 
  tabyl(ratio_binned_collapsed, sa4_name_2016) 
#chisq.test(si_by_areaSA12016_2016_2021_wider_table_sa1)$expected  
#4 cells less than 5.  4x6 cells total (24) (excluding inner SA4s and never served areas.)
# 16% below 5, there assumptions met.  


si_by_areaSA12016_2016_2021_wider_table_sa1_appendix_table <- si_by_areaSA12016_2016_2021_wider_table_sa1 %>%
  select(ratio_binned_collapsed, Inner, `Inner South`, `Inner East`, `North East`, `North West`, `Outer East`, `South East`, West, `Mornington Peninsula`) %>% 
  untabyl() %>%
  adorn_totals(where = c("row", "col")) %>%
  adorn_percentages(denominator = "all") %>% 
  adorn_pct_formatting() %>%
...
# 4 cells <5. 
# Total cells = 4 x 6 = 24
# 4/24 = 16.7%. 20% limit on cells <5 assumption met. 
...
si_by_area_2016_2021_wider$ratio_binned_collapsed <- factor(
  if_else(si_by_area_2016_2021_wider$ratio_binned == "New service", "New service", 
          if_else(si_by_area_2016_2021_wider$ratio_binned == "Increased 30% or more", "Increased 30% or more", 
                  if_else(si_by_area_2016_2021_wider$ratio_binned ==  "Increased 10 to 30%" | 
                            si_by_area_2016_2021_wider$ratio_binned == "Increased 5 to 10%", "Increased 5 to 30%", 
                          if_else(si_by_area_2016_2021_wider$ratio_binned == "Increased 3 to 5%" | 
                                    si_by_area_2016_2021_wider$ratio_binned == "Increased 1 to 3%", "Increased 1 to 5%", 
                                  if_else(si_by_area_2016_2021_wider$ratio_binned == "Within 1%", "Within 1%", 
                                          if_else(si_by_area_2016_2021_wider$ratio_binned == "Reduced 1 to 3%" |
                                                  si_by_area_2016_2021_wider$ratio_binned == "Reduced 3 to 10%" | 
                                                  si_by_area_2016_2021_wider$ratio_binned == "Reduced 10% or more" |
                                                  si_by_area_2016_2021_wider$ratio_binned == "Service withdrawn (magenta)",
                                                  "Service reduced or withdrawn", 
                                                  "Never served")
                                  )
                          )
...
si_by_area_2016_2021_wider$ratio_binned_collapsed <- si_by_area_2016_2021_wider$ratio_binned_collapsed %>%
  fct_rev()
si_by_area_2016_2021_wider$sa4_name_2021 <- si_by_area_2016_2021_wider$sa4_name_2021 %>%
  fct_rev()
...
ggbarstats(
  ...
  theme(
  legend.position="bottom") +
  scale_fill_manual(values = c("#3A65A3", "#6D9DC4", "#C2DFEB", "#FCE99D", "#EFA15C", "#BA3832", "black")) 

 

si_by_area_2016_2021_wider_table_sa1_appendix_table <- si_by_area_2016_2021_wider %>% 
  st_drop_geometry() %>%
  na.omit() %>% 
  filter(very_high_needs_zero_or_very_low_supply == TRUE) %>% 
  tabyl(ratio_binned_collapsed, sa4_name_2021) %>%
  select(ratio_binned_collapsed, Inner, `Inner South`, `Inner East`, `North East`, `North West`, `Outer East`, `South East`, West, `Mornington Peninsula`) %>% 
  untabyl() %>%
  adorn_totals(where = c("row", "col")) %>%
  adorn_percentages(denominator = "all") %>% 
  adorn_pct_formatting() %>%
  adorn_ns() %>%
  kable(caption = "Greater Melbourne: SA1s with Very High needs but Very Low or Zero supply in 2021, by SA4 and change in SI since 2016",
      align = "lrrrrrrrrrr",
      col.names = c("Change", 
                   "Inner", "Inner South", "Inner East",
                   "North East", "North West",
...
smaller_table <- si_by_area_2016_2021_wider %>% 
  st_drop_geometry() %>%
  na.omit() %>% 
  filter(very_high_needs_zero_or_very_low_supply == TRUE) %>%
    filter(sa4_name_2021 != "Inner South" & 
         sa4_name_2021 != "Inner East" &
         sa4_name_2021 != "Inner") %>%
    mutate(sa4_name_2021 = fct_drop(sa4_name_2021)) %>%
  tabyl(ratio_binned_collapsed, sa4_name_2021)
  chisq.test(smaller_table)$expected
# 4 cells <5. 
# Total cells = 7 x 6 = 42
# 4/42 = 9.5%. 20% limit on cells <5 assumption met. 
...
pcol <- si_by_area_2016_2021_wider %>%
  ...
  mutate(perc=population/sum(population)) %>% 
  ggplot() + 
  geom_col(aes(ratio_binned_collapsed, population, fill = ratio_binned_collapsed)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned_collapsed, population + 150000, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 2) +
  coord_flip() + 
    scale_fill_manual(values = c("#3A65A3", "#6D9DC4", "#C2DFEB", "#FCE99D", "#EFA15C", "#BA3832", "black")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_mornington <- left_join(data.frame(ratio_binned_collapsed=levels(si_by_area_2016_2021_wider$ratio_binned_collapsed)),
  si_by_area_2016_2021_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "Mornington Peninsula") %>% 
  filter(very_high_needs_zero_or_very_low_supply == TRUE) %>%
    aggregate(population ~ ratio_binned_collapsed, sum)) %>%
  ...
  geom_col(aes(ratio_binned_collapsed, population, fill = ratio_binned_collapsed)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned_collapsed, population + 100, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 2) +
  coord_flip() +
  scale_fill_manual(values = c("#3A65A3", "#6D9DC4", "#C2DFEB", "#FCE99D", "#EFA15C", "#BA3832", "black"))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_inner <- left_join(data.frame(ratio_binned_collapsed=levels(si_by_area_2016_2021_wider$ratio_binned_collapsed)),
  si_by_area_2016_2021_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "Inner") %>% 
  filter(very_high_needs_zero_or_very_low_supply == TRUE) %>%
    aggregate(population ~ ratio_binned_collapsed, sum)) %>%
...
## Inner East does not have any high needs, very low or zero supply areas


pcol_inner_south <- left_join(data.frame(ratio_binned_collapsed=levels(si_by_area_2016_2021_wider$ratio_binned_collapsed)),
  si_by_area_2016_2021_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "Inner South") %>% 
  filter(very_high_needs_zero_or_very_low_supply == TRUE) %>%
    aggregate(population ~ ratio_binned_collapsed, sum)) %>%
...
pcol_north_east <- left_join(data.frame(ratio_binned_collapsed=levels(si_by_area_2016_2021_wider$ratio_binned_collapsed)),
  si_by_area_2016_2021_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "North East") %>% 
  filter(very_high_needs_zero_or_very_low_supply == TRUE) %>%
    aggregate(population ~ ratio_binned_collapsed, sum)) %>%
...
pcol_north_west <- left_join(data.frame(ratio_binned_collapsed=levels(si_by_area_2016_2021_wider$ratio_binned_collapsed)),
  si_by_area_2016_2021_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "North West") %>% 
  filter(very_high_needs_zero_or_very_low_supply == TRUE) %>%
    aggregate(population ~ ratio_binned_collapsed, sum)) %>%
...
pcol_outer_east <- left_join(data.frame(ratio_binned_collapsed=levels(si_by_area_2016_2021_wider$ratio_binned_collapsed)),
  si_by_area_2016_2021_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "Outer East") %>% 
  filter(very_high_needs_zero_or_very_low_supply == TRUE) %>%
    aggregate(population ~ ratio_binned_collapsed, sum)) %>%
...
pcol_south_east <- left_join(data.frame(ratio_binned_collapsed=levels(si_by_area_2016_2021_wider$ratio_binned_collapsed)),
  si_by_area_2016_2021_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "South East") %>% 
  filter(very_high_needs_zero_or_very_low_supply == TRUE) %>%
    aggregate(population ~ ratio_binned_collapsed, sum)) %>%
...
pcol_west <- left_join(data.frame(ratio_binned_collapsed=levels(si_by_area_2016_2021_wider$ratio_binned_collapsed)),
  si_by_area_2016_2021_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "West") %>% 
  filter(very_high_needs_zero_or_very_low_supply == TRUE) %>%
    aggregate(population ~ ratio_binned_collapsed, sum)) %>%
...
Greater_Melbourne_2016_2021_ratio_map_very_high_very_low_zero_2021 <- Greater_Melbourne_2016_2021_ratio_map +
  ggspatial::annotation_scale(location = 'br') + 
  ggspatial::annotation_north_arrow(location = 'tr') + 
  inset_element(pcol, left = 0.0, bottom = 0.0, right = 0.3, top =0.25) +
  inset_element(pcol_mornington, left = 0.58, bottom = 0.03, right = 0.78, top =0.18) +
...
```{r Greater_Melbourne_needs_gap_map_figure, eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.cap="Greater Melbourne Transport Supply groupings overlayed with SA1s with very high transport need areas with zero or very low public Transport Supply (red).", fig.ncol = 2, out.width = "50%", fig.align = "center", fig.subcap = c("2016", "2021")}

Greater_Melbourne_2016_2021_ratio_map_very_high_very_low_zero_2021

```


<!---
There was a `r prmisc::print_chi2(chisq.test(si_by_areaSA12016_2016_2021_wider %>% st_drop_geometry() %>% na.omit() %>% filter(very_high_needs_zero_or_very_low_supply == TRUE) %>% filter(sa4_name_2016 != "Inner South" & sa4_name_2016 != "Inner East" & sa4_name_2016 != "Inner") %>% mutate(sa4_name_2016 = fct_drop(sa4_name_2016)) %>% mutate(ratio_binned_collapsed = fct_drop(ratio_binned_collapsed)) %>% tabyl(ratio_binned_collapsed, sa4_name_2016)))`

Greater_Melbourne_2016_needs_gap_SA4_service_change

Table \ref{Greater_Melbourne_2021_needs_gap_SA4_service_change} shows 
how service levels had changed for those residents living in SA1 areas in 2021 
that had Very High needs but Very Low or Zero service, summarised by SA4. There is a significant difference across SA4s (`r prmisc::print_chi2(chisq.test(si_by_area_2016_2021_wider %>% st_drop_geometry() %>% na.omit() %>% filter(very_high_needs_zero_or_very_low_supply == TRUE) %>% filter(sa4_name_2021 != "Inner South" & sa4_name_2021 != "Inner East" & sa4_name_2021 != "Inner") %>% mutate(sa4_name_2021 = fct_drop(sa4_name_2021)) %>% tabyl(ratio_binned_collapsed, sa4_name_2021)))`). 


Of those people living in SA1s that Very High needs but Very Low or Zero service in 2016, those living in the North West, South East and West were more likely to have received some new or additional transit service by 2021. For those living in the Outer East, Inner South and Mornington Peninsula service levels were more likely to have stayed within one percent or been reduced, or (for `r si_by_areaSA12016_2016_2021_wider_table[4,'Outer East'] %>% sum()` people living in the Outer East (`r label_percent(accuracy = 0.1)(si_by_areaSA12016_2016_2021_wider_table[4,'Outer East'] %>% sum()/si_by_areaSA12016_2016_2021_wider_table[,'Outer East'] %>% sum())`)) had their service reduced or withdrawn.   



Table \ref{Greater_Melbourne_2021_needs_gap_SA4_service_change} shows 
how service levels had changed for those residents living in SA1 areas in 2021 
that had Very High needs but Very Low or Zero service, summarised by SA4. There is a significant difference across SA4s (`r prmisc::print_chi2(chisq.test(smaller_table))`). Of those people living in SA1s that Very High needs but Very Low or Zero service in 2016, those living in the North West, South East and West were more likely to have received some new or additional transit service by 2021. For those living in the Outer East, Inner South and Mornington Peninsula service levels were more likely to have stayed within one percent or been reduced, or (for `r si_by_areaSA12016_2016_2021_wider_table[4,'Outer East'] %>% sum()` people living in the Outer East (`r label_percent(accuracy = 0.1)(si_by_areaSA12016_2016_2021_wider_table[4,'Outer East'] %>% sum()/si_by_areaSA12016_2016_2021_wider_table[,'Outer East'] %>% sum())`)) had their service reduced or withdrawn.   
