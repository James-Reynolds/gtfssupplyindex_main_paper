...
in many locations 
is to provide some mobility 
for those who cannot otherwise drive themselves, 
so they can access activities and services 
...
suggesting that there may be an increasing need 
...
As well, 
it is unclear whether the spatial patterns identified for Melbourne in 2006
have changed in the intervening years, 
or if they are representative of other places. 
...
with tools for undertaking social needs-gaps analysis with GTFS datasets. 
...
and provide a rating out of 100 
...
which provides metrics for grouping many aspects of a transit system 
into five categories (very good to not very good) 
...
A generalized form of the Supply Index (SI) equation, adapted from @currie2010identifying, is:
...
although there were also some pockets in the middle suburbs 
in the west, north and south east. 
Overall,
...
may be partly why 
it has not been more widely adopted. 
...
When using a case research approach there is also a need 
to address the "duality criterion", 
being seeking generalisable findings 
...
While this study does seek findings about changes in spatial patterns 
of social needs and transit supply that are generalisable to more than just Melbourne, 
a lack of 2006 results for other cities may limit 
the extent to which these findings might be confidently considered 
representative of changes in other places[^2].
...
given that the authors are familiar with the surrounding geography 
...
ranging from SA1s (\~400 people) to SA4s (parts of a city or region)[@ABSmaps]. 
...
The same Transport Supply categorizations 
have been used as in @currie2010identifying[^3]. 
As well, this study adopts a similar approach 
...
cannot be exactly replicated[^5] based on weighting both the IRSAD index and the transport need index by the total population of each SA1. These were then standardised and grouped, as per the six groups used by @currie2010identifying[^6].
...
The difference between the share of areas of interest in each category in 2006 (CCDs),
2016 and 2021 (SA1s) is also statistically significant
...
Greater Melbourne's population increased by `r label_percent(accuracy=1)(sum(melbourne_comparison_2006_2021[,9])/sum(melbourne_comparison_2006_2021[,3])-1)` between 2006 
(`r sum(melbourne_comparison_2006_2021[,3]) %>% format(big.mark = ",")`)
and 2021 (`r sum(melbourne_comparison_2006_2021[,3]) %>% format(big.mark = ",")`).
However, the number of residents living in areas with Zero Supply rose by 
...
(`r label_percent(accuracy=0.1)(sum(melbourne_comparison_2006_2021[1:2,3])/sum(melbourne_comparison_2006_2021[,3]))`) in 2006 to 
...
in 2021, 
while those with supply below the average (i.e. Zero, Very Low, Low or Below Average) rose by 
...
inset_element(pcol, left = 0.0, bottom = 0.0, right = 0.40, top =0.30) +
  inset_element(pcol_mornington, left = 0.56, bottom = 0.03, right = 0.86, top =0.17) +
  inset_element(pcol_south_east, left = 0.82, bottom = 0.03, right = 1.12, top =0.17) +
  inset_element(pcol_outer_east, left = 0.82, bottom = 0.686, right = 1.12, top =0.83) +
  inset_element(pcol_north_east, left = 0.6, bottom = 0.80, right = 0.9, top =0.95)  +
  inset_element(pcol_north_west, left = 0.0, bottom = 0.81, right = 0.3, top =0.96) +
  inset_element(pcol_west, left = 0.00, bottom = 0.32, right = 0.3, top =0.47) +
  inset_element(pcol_inner, left = 0.17, bottom = 0.32, right = 0.47, top =0.47) +
  inset_element(pcol_inner_south, left = 0.33, bottom = 0.25, right = 0.63, top =0.4) +
  inset_element(pcol_inner_east, left = 0.54, bottom = 0.59, right = 0.84, top =0.72) 
...
inset_element(pcol, left = 0.0, bottom = 0.0, right = 0.40, top =0.30) +
  ...
  inset_element(pcol_outer_east, left = 0.78, bottom = 0.686, right = 1.08, top =0.83) +
  ...
  inset_element(pcol_inner_south, left = 0.33, bottom = 0.25, right = 0.63, top =0.4) +
  inset_element(pcol_inner_east, left = 0.59, bottom = 0.59, right = 0.89, top =0.72) 
...
across the inner, middle and outer suburbs, respectively.
...
Average SI scores for SA1s across all of Greater Melbourne increased from
...
service levels had stayed within one percent of the 2016 
(making up 
...
the (labeled) SA1s with particularly low needs scores and Very High supply
are mostly in inner parts of the city^[CBD, Richmond and Collingwood. 
Although Broadmeadows and Box Hill are notable exceptions,
being located on rail lines in the middle to outer suburbs], 
while those with particularly high needs and Very Low supply are 
in the outer suburbs. There is a significant,
but only weakly positive correlation
between the SI and Combined Needs Index scores for both 2016 
...
did not appear to have large gaps between need and supply.].

### Needs-gap and service level changes

```{r Greater_Melbourne_2016_needs_gap_SA4_service_change, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.dim=c(7.5,7.5), fig.cap="Greater Melbourne: change in transit supply between 2016 and 2021 for SA1s with Very High needs but Very Low or Zero supply in 2016"}
...
pcol <- si_by_areaSA12016_2016_2021_wider %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned_collapsed, perc+0.06, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 2) +
  coord_flip() + 
  scale_fill_manual(values = c("#3A65A3", "#C2DFEB", "#EFA15C", "#BA3832", "black")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_mornington <- left_join(data.frame(ratio_binned_collapsed=levels(si_by_areaSA12016_2016_2021_wider$ratio_binned_collapsed)),
 ...
 Greater_MelbourneSA12016_2016_2021_ratio_map +
  ggspatial::annotation_scale(location = 'br') + 
  ggspatial::annotation_north_arrow(location = 'tr') +
inset_element(pcol, left = 0.0, bottom = 0.0, right = 0.55, top =0.30) +
  ...
  in8et_element(pcol_north_west, left = 0.0, bottom = 0.83, right = 0.3, top =0.96) +
...
for SA1s with Very High needs but Very Low or Zero supply in 2016.
...
shows how much the transit service level changed for populations
living in SA1s
with Very High needs
but Very Low or Zero supply in 2016 by SA4.
...
people who were living elsewhere Greater Melbourne in 2016
...
```{r Bus_798, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.ncol = 2, out.width = "50%", fig.align = "center", fig.show="hold",  fig.cap="Cranbourne and Selandra Rise", fig.subcap=c("Bus Route 798 as originally introduced in 2014 (Source: Delbosc et al (2015))", "Current (2024) route map (Source: PTV)")}
