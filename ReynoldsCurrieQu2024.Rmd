...
pcol <- si_by_areaSA12016_2016_2021_wider %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned_collapsed, perc+0.07, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 2) +
  coord_flip() + 
  scale_fill_manual(values = c("#3A65A3", "#C2DFEB", "#EFA15C", "#BA3832", "black")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_inner <- left_join(data.frame(ratio_binned_collapsed=levels(si_by_areaSA12016_2016_2021_wider$ratio_binned_collapsed)),
  ...
  scale_y_continuous(limits = c(0,1)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned_collapsed, perc+0.15, label = glue::glue("{label_percent(accuracy = 1)(perc)}")), size = 2) +
  coord_flip() +
   scale_fill_manual(values = c("#3A65A3", "#C2DFEB", "#EFA15C", "#BA3832", "black")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
Greater_Melbourne_2016_2021_ratio_map_very_high_very_low_zero_2016 <- Greater_MelbourneSA12016_2016_2021_ratio_map +
  ggspatial::annotation_scale(location = 'br') + 
  ggspatial::annotation_north_arrow(location = 'tr') + 
  inset_element(pcol, left = 0.05, bottom = 0.2, right = 0.60, top =0.4) +
  inset_element(pcol_mornington, left = 0.2, bottom = 0.00, right = 0.35, top =0.15) +
  inset_element(pcol_south_east, left = 0.65, bottom = 0.05, right = 0.95, top =0.20) +
  inset_element(pcol_outer_east, left = 0.70, bottom = 0.7, right = 1.0, top =0.85) +
  inset_element(pcol_north_east, left = 0.58, bottom = 0.85, right = 0.83, top =1.0)  +
