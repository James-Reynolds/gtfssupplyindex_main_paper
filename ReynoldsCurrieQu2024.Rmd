...
An example of how the social needs-gap analysis and gtfssupplyindex package might facilitate such a demonstration relates to the `r si_by_area_2016_2021_wider_table[11,4] %>% as.numeric() %>% format(big.mark = ",")` people living in the North East in 2021 who had some transit service in 2016 but none in 2021, yet have very high needs, noted above. Figure \ref{fig: The analysis presented here might provide a strong case against the withdrawal of service, and highlights spatial areas worthy of further exploration.

```{r map_headway, eval = FALSE, echo = FALSE, warning=FALSE, message=FALSE}


##Based on https://cran.r-project.org/web/packages/tidytransit/vignettes/frequency.html
map_headway <- function(
    mapping_date = lubridate::ymd(231010),
    gtfs =  action_buses_20230925,
    start_time = 7*3600,
    end_time = 9*3600, 
    EPSG_for_transform = 28355, 
    min_stop_departures = 10, 
    median_headways_limit = 100000){
  ...
  service_pattern_summary <- gtfs$trips %>%
    left_join(gtfs$.$servicepatterns, by="service_id", copy = TRUE) %>%  
    left_join(shape_lengths, by="shape_id", copy = TRUE) %>%
    left_join(gtfs$stop_times, by="trip_id", copy = TRUE) %>% 
  ...
  service_pattern_summary <- gtfs$.$dates_servicepatterns %>% 
    ...
    left_join(service_pattern_summary, by="servicepattern_id", copy = TRUE)

## Select service_ids for supplied date
  servicepattern_ids <- gtfs$.$dates_servicepatterns  %>% 
    filter(date == mapping_date) %>% 
    pull(servicepattern_id)

service_ids <- gtfs$.$servicepattern %>% 
  filter(servicepattern_id == servicepattern_ids) %>% 
  pull(service_id)
...
#    knitr::kable()
  ...
  am_stop_freq <- get_stop_frequency(gtfs, start_time = start_time, end_time = end_time, service_ids = service_ids, by_route = TRUE)

  output_list <- list(stop_frequency_table = am_stop_freq)

  am_route_freq <- get_route_frequency(gtfs, service_ids = service_ids, 
                                     start_time = start_time, end_time = end_time) 
  output_list$am_route_freq <- am_route_freq
...
# get_route_geometry needs a gtfs object that includes shapes as simple feature data frames
  ...
  routes_sf <- routes_sf %>% 
    inner_join(am_route_freq, by = 'route_id', copy = TRUE)

# convert to an appropriate coordinate reference system
  routes_sf_crs <- sf::st_transform(routes_sf, EPSG_for_transform) 
  output_list$plot_high_frequency <-  routes_sf_crs %>% 
                          filter(median_headways < median_headways_limit) %>%
                          ggplot() + 
                          geom_sf(aes(colour=as.factor(median_headways))) + 
                          labs(color = "Headways", x = "", y = "") + 
...
#5) Map Departures by Stop and Route
  ...
  output_list$routes_sf_buffer <- routes_sf_buffer
  output_list$plot_routes <- 
                        routes_sf_buffer %>% 
                          ggplot() + 
                          geom_sf(colour = alpha("white", 0), fill = alpha("red",0.2)) +
                          theme_bw()

  output_list$departures_by_stop <- gtfs$stops %>% 
                          inner_join(am_stop_freq, by = "stop_id", copy = TRUE) %>% 
                          #  filter(n_departures > 50) %>% 
                          select(stop_id, stop_name, n_departures, mean_headway) %>% 
                          arrange(n_departures)
  
  am_stop_name_departures <- left_join(gtfs$stops, am_stop_freq, by="stop_id", copy = TRUE)
  am_stop_name_departures <- am_stop_name_departures %>%  
    group_by(stop_name) %>%
    transmute(total_departures = sum(n_departures, na.rm=TRUE))

  am_stop_name_departures <- am_stop_name_departures %>%
    filter(total_departures > min_stop_departures)

  output_list$am_stop_name_departures <- am_stop_name_departures
  
  output_list$plot_route_and_departures <- 
   ggplot() + 
...
## filter feed to only include North West and North East SA4s
...
## convert to list of tidygtfs objects
ptv_210805_gtfs_north_east <- ptv_210805_gtfs_north_east %>% 
  set_servicepattern()
ptv_210805_gtfs_north_west <- ptv_210805_gtfs_north_west %>% 
  set_servicepattern()


map_victoria_2021 <- map_headway(
mapping_date = lubridate::ymd(210810),
gtfs =  ptv_210805_gtfs,
start_time = 2*3600,
end_time = 32*3600,
EPSG_for_transform = 28355,
min_stop_departures = 100,
median_headways_limit = 5000)
...
#action_buses_20230925 <- tidytransit::read_gtfs("data/action-buses/20230925/gtfs.zip") %>% 
#  set_servicepattern()



#canberra_route_map_output_bus_2023 <- map_headway(
#mapping_date = lubridate::ymd(231010),
#gtfs =  action_buses_20230925,
#start_time = 7*3600,
#end_time = 9*3600,
...
```{r north_east_changes, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.dim = c(7.5,7.5), fig.cap="Part of the North East SA4 region (centred on South Morang), showing change in SI score between 2016 and 2021 for SA1s with Very High needs but Very Low or Zero supply. Labels show population of SA1s for which service was withdrawn "}
...
scale_fill_manual(values = c("#5C66A0", "#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "black")) +
  geom_sf(data=melbourne_sd07aust_region, 
          colour="black", 
          fill=NA) +
  geom_sf(data=melbourne_railway_trips_shape_2021, linetype = "twodash", size = 0.1) + 
...
#    coord_sf(ylim = c(-37.23, -37.85), 
#          xlim = c(144.85, 145.55))
