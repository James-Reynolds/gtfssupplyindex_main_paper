...
pcol_mornington <- left_join(data.frame(ratio_binned_collapsed=levels(si_by_areaSA12016_2016_2021_wider$ratio_binned_collapsed)),
  ...
  mutate(ratio_binned_collapsed = factor(ratio_binned_collapsed, 
   levels = c("New service or increase of 30% or more", 
      "Increased 1 to 30%", 
      "Within 1%",
      "Service reduced or withdrawn",
      "Never served"))) %>%
...
pcol_inner <- left_join(data.frame(ratio_binned_collapsed=levels(si_by_areaSA12016_2016_2021_wider$ratio_binned_collapsed)),
  si_by_areaSA12016_2016_2021_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2016 == "Inner") %>% 
  filter(very_high_needs_zero_or_very_low_supply == TRUE) %>%
    aggregate(population ~ ratio_binned_collapsed, sum)) %>%
...
###No inner east SA1s in very high needs and very low zero supply

pcol_inner_south <- left_join(data.frame(ratio_binned_collapsed=levels(si_by_areaSA12016_2016_2021_wider$ratio_binned_collapsed)),
  si_by_areaSA12016_2016_2021_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2016 == "Inner South") %>% 
  filter(very_high_needs_zero_or_very_low_supply == TRUE) %>%
    aggregate(population ~ ratio_binned_collapsed, sum)) %>%
...
```{r Greater_Melbourne_2021_needs_gap_SA4_service_change, fig.show="hold", eval = FALSE, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.dim = c(7.5,7.5), fig.cap="Greater Melbourne SA1s with Very High transport need and Zero or Very Low supply in 2021, population by change in SI score 2016 to 2021, across SA4."}
