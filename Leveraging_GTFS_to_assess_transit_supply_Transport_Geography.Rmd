...
```{r Greater_Melbourne_2016_plot_SA12016, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, fig.cap="Greater Melbourne, Transit Supply for the weeks starting the date of 2016 and 2021 census (2016 SA1 boundaries), overlayed with: 2006 Greater Melbourne boundary (black); middle/outer and inner/middle suburb boundaries (grey); and suburban railway lines (dashed)"}
...
```{r Greater_Melbourne_2016_2021_plot_SA12016, fig.show="hold", eval=FALSE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, fig.cap="Greater Melbourne, Transit Supply for the weeks starting the date of 2016 and 2021 census (2016 SA1 boundaries), overlayed with: 2006 Greater Melbourne boundary (black); middle/outer and inner/middle suburb boundaries (grey); and suburban railway lines (dashed)"}
...
si_by_area_2016_2021 <- add_row(
  ...
  melbourne_si_SA12016_21_census_week_aggregated$si_by_area %>% 
    st_drop_geometry())
names(si_by_area_2016_2021) <- c("sa1_code_2016", "SI", "transit_supply", "Year")
...
```{r Greater_Melbourne_SA1_2016_2021_table_and_bar_chart_SA12016, fig.show="hold", eval = FALSE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.height = 8, fig.cap="Greater Melbourne: distribution of Transit Supply to SA1s (2016 boundaries)(top) and population (bottom) in 2016 and 2021"}

si_by_area_2016_2021_transit_supply_wider <- si_by_area_2016_2021 %>% 
  st_drop_geometry() %>%
  na.omit() %>% 
  select(sa1_code_2016, Year, transit_supply) %>%
  pivot_wider(names_from = "Year", 
              values_from = "transit_supply") %>% 
  clean_names()
...
sa1_ur_agep_2021 <- right_join(
  sa1_ur_agep_2021, 
  CG_SA1_2016_SA1_2021
)
#divide 2021 sa1_code_2021 populations by correspondence to get sa1_code_2016 2021 populations
sa1_ur_agep_2021$population_SA12016s_2021 <- sa1_ur_agep_2021$total * sa1_ur_agep_2021$ratio_from_to
#aggregate by sa1_code_2016
sa1_ur_agep_2021_SA12016 <- sa1_ur_agep_2021 %>% aggregate(population_SA12016s_2021 ~ sa1_code_2016, sum)
sa1_ur_agep_2021_SA12016$population_SA12016s_2021 <-  sa1_ur_agep_2021_SA12016$population_SA12016s_2021 %>% 
  round(digits = 0)

#join into population
population_transit_supply_sa1_2016_2021 <- left_join( 
  population_transit_supply_sa1_2016_2021, 
  left_join(
    areas_of_interest %>% st_drop_geometry(), 
    sa1_ur_agep_2021_SA12016 %>% select(sa1_code_2016, total)) %>% 
    as_tibble()
  )

names(population_transit_supply_sa1_2016_2021) <- c("sa1_code_2016", 
                                             "transit_supply_2016", 
                                             "transit_supply_2021",
                                             "population_2016",
                                             "population_2021")
...
```{r Greater_Melbourne_2016_2021_scatterplot_SA12016, fig.show="hold", eval = FALSE, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, include = TRUE, fig.fullwidth = TRUE, fig.height = 8, fig.cap="Greater Melbourne: SI scores in 2016 and 2021 by SA1 (2021 boundaries), adjusted to set those <1 equal to 1 (for visual clarity)"}
...
```{r Greater_Melbourne_2016_2021_ratio_table_SA12016, fig.show="hold", out.width = "90%", eval  = FALSE, echo = FALSE, warning=FALSE, message=FALSE, cache = TRUE, include = TRUE, fig.height = 8, fig.cap="Greater Melbourne: changes in SI between 2016 and 2021, number of SA1s by SA4"}
...
```{r Greater_Melbourne_2016_2021_ratio_map_SA12016, fig.show="hold", out.width = "90%", eval  = FALSE, echo = FALSE, warning=FALSE, message=FALSE, cache = TRUE, include = TRUE, fig.cap="Greater Melbourne: changes in SI between 2016 and 2021, by SA1"}
