...
```{r map_headway, echo = FALSE, warning=FALSE, message=FALSE}
...
map_headway <- function(
  ...
  service_pattern_summary <- gtfs$trips %>%
    left_join(gtfs$.$servicepatterns, by="service_id") %>%     left_join(shape_lengths, by="shape_id") %>%
...
```{r outer_east_changes, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.dim = c(7.5,7.5), fig.cap="Greater Melbourne SA1s with Very High transport need and Zero or Very Low supply in 2021, population by change in SI score 2016 to 2021, across SA4."}
