...
write.csv(si_SA12021_21_census_week_aggregated, "results/Greater_Melbourne/si_SA12021_21_census_week_aggregated.csv")
...
###2021 Greater Melbourne CCD
###Load results from CSV (Precalculated above) and make first column character
si_CCD2006_21_census_week_aggregated <- read.csv( "results/Greater_Melbourne/si_CCD2006_21_census_week_aggregated.csv") %>% 
  as_tibble()
si_CCD2006_21_census_week_aggregated <- si_CCD2006_21_census_week_aggregated[,2:3]

si_CCD2006_21_census_week_aggregated$area_id <- si_CCD2006_21_census_week_aggregated$area_id %>% 
  as.character()
...
si_CCD2006_21_census_week_aggregated <- full_join(
  melbourne_CCD,
  si_CCD2006_21_census_week_aggregated, 
  by = join_by(CD_CODE_2006  == area_id))
si_CCD2006_21_census_week_aggregated[is.na(si_CCD2006_21_census_week_aggregated)] <- 0
...
names(si_by_area_dataframe) <- c("area_of_interest", "SI", "Transit Supply")
...
si_CCD2006_21_census_week_aggregated_thresholds <- set_thresholds(si_by_area_dataframe = si_CCD2006_21_census_week_aggregated)
si_CCD2006_21_census_week_aggregated <- si_CCD2006_21_census_week_aggregated_thresholds[[1]]
si_CCD2006_21_census_week_aggregated$Year <- 2021

si_CCD2006_21_census_week_aggregated <- left_join(
  melbourne_CCD, 
  si_CCD2006_21_census_week_aggregated, 
  join_by(CD_CODE_2006 == area_of_interest))

plot_2021_Melbourne_CCD <- ggplot()+ 
  geom_sf(data=si_CCD2006_21_census_week_aggregated %>% na.omit(),
...
<NEED TO ADD OTHER CITIES. MIGHT BE BEST TO CHANGE ALL OF THE NEXT CHUCK INTO A FUNCTION TO THEN APPLY TO EACH CITY IN TURN>
...
###Load results from CSV (Precalculated above) and make first column character
si_SA12021_21_census_week_aggregated <- read.csv( "results/Greater_Melbourne/si_SA12021_21_census_week_aggregated.csv") %>% 
  as_tibble()
...
si_SA12021_21_census_week_aggregated <- left_join(
  absmapsdata::sa12021 %>% 
    filter(gcc_name_2021 == "Greater Melbourne") %>% 
    select(sa1_code_2021),
  si_SA12021_21_census_week_aggregated)

plot_2021_Melbourne <- ggplot()+ 
  geom_sf(data=si_SA12021_21_census_week_aggregated %>% na.omit(),
          aes(fill = `Transit Supply`), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
Greater_Melbourne_2021_needs_gap_SA1_count <- tabyl(combined_needs_index_2021_sa1_greater_melbourne, `Transit Supply`,  combined_needs_index_binned)
...
Greater_Melbourne_2021_needs_gap_SA1_population <- tapply(combined_needs_index_2021_sa1_greater_melbourne$population,
       list(combined_needs_index_2021_sa1_greater_melbourne$`Transit Supply`, combined_needs_index_2021_sa1_greater_melbourne$combined_needs_index_binned), 
       sum) %>% 
  as.data.frame() %>%
  rownames_to_column() 
