...
explored
spatial gaps between the social need for transport 
and what transit is supplied. 
This work presented a transit Supply Index (SI) 
and compared it to 
measures of social need for transport 
across a few Australian cities.  
...
As well, at the time applying such methodologies elsewhere 
would appear likely to have required 
a bespoke data collection, 
cleaning and analysis effort. 
Nowadays, however, 
...
and allowing comparison 
to the 2006 result reported in @currie2010identifying.
...
and a combination of the two. Figure  \ref{fig:Currie_map_gap} 
reproduces the resultant map 
identifying areas with very high transport needs, 
...
The results indicated various areas 
where service gaps might be of concern, 
...
```{r run_for_all_modes_ACT_bus_2016_census_day, eval = TRUE, echo = FALSE}
...
```{r run_for_all_modes_ACT_bus_2021_census_day, eval = TRUE, echo = FALSE}
...
```{r run_for_all_modes_ACT_LRT_2021_census_day, eval = TRUE, echo = FALSE}
...
```{r run_for_all_modes_Adelaide_2016_census_day, eval = TRUE, echo = FALSE}
...
```{r run_for_all_modes_Adelaide_2021_census_day, eval = TRUE, echo = FALSE}
...
```{r run_for_all_modes_Tassie_Burnie_2016_census_day, eval = TRUE, echo = FALSE}
...
```{r run_for_all_modes_Tassie_Burnie_2021_census_day, eval = TRUE, echo = FALSE}
...
```{r run_for_all_modes_Tassie_Hobart_2016_census_day, eval = TRUE, echo = FALSE}
...
```{r run_for_all_modes_Tassie_Hobart_2021_census_day, eval = TRUE, echo = FALSE}
...
```{r run_for_all_modes_Tassie_Launceston_2016_census_day, eval = TRUE, echo = FALSE}
...
```{r run_for_all_modes_Tassie_Launceston_2021_census_day, eval = TRUE, echo = FALSE}
...
```{r run_for_all_modes_SEQ_2016_census_day, eval = TRUE, echo = FALSE}
...
```{r run_for_all_modes_SEQ_2021_census_day, eval = TRUE, echo = FALSE}
...
```{r run_for_all_modes_Perth_2016_census_day, eval = TRUE, echo = FALSE}
...
```{r run_for_all_modes_transperth_2021_census_day, eval = TRUE, echo = FALSE}
...
plot_2021 <- ggplot()+ 
  geom_sf(data=si_by_area_2016_2021 %>% 
            filter(Year == 2021) %>% 
            na.omit(),
          aes(fill = SI_binned), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
...
```{r Greater_Melbourne_2016_2021_scatterplot, fig.show="hold", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, include = TRUE, fig.fullwidth = TRUE, fig.cap="SI scores by SA3, census day 2016 and 2021"}
