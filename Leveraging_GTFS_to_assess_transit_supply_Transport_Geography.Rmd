...
<!---
Figure \ref{fig:Greater_Melbourne_2016_2021_plot} shows the distribution of Transport Supply categories across Greater Melbourne in the week of the 2016 and 2021 censuses, using the SA1 2021 boundaries. The overall spatial patterns appear generally similar. Some locations that have higher categories of transport supply in 2021 are apparent, including around Werribee (outer south-west), Craigieburn (outer north) and Lillydale (outer north-west).
--->
...
```
Table \ref{tab:Greater_Melbourne_SA1_2016_2021_table_and_bar_chart} compares the share of SA1s and resident population in each transport supply category. Differences are statistically significant across the number of SA1s (`r prmisc::print_chi2(comparison_2016_2021 %>% select(SA12021s_2016, SA1s_2021) %>% as.matrix())`) and population share (`r prmisc::print_chi2(comparison_2016_2021 %>% select(population_SA12021s_2016, population_2021) %>% as.matrix())`). The number of SA1s with Zero or Very Low transport supply fell from `r format(sum(comparison_2016_2021[1:2,2]), big.mark = ",")` (`r label_percent(accuracy=0.1)(sum(comparison_2016_2021[1:2,2])/sum(comparison_2016_2021[,2]))`) to `r format(sum(comparison_2016_2021[1:2,4]), big.mark = ",")` (`r label_percent(accuracy=0.1)(sum(comparison_2016_2021[1:2,4])/sum(comparison_2016_2021[,4]))`), but number of people with zero supply rose from `r format(sum(comparison_2016_2021[1:2,3]), big.mark = ",")` (`r label_percent(accuracy=0.1)(sum(comparison_2016_2021[1:2,3])/sum(comparison_2016_2021[,3]))`) to `r format(sum(comparison_2016_2021[1:2,5]), big.mark = ",")` (`r label_percent(accuracy=0.1)(sum(comparison_2016_2021[1:2,5])/sum(comparison_2016_2021[,5]))`). This represents a `r label_percent(accuracy=0.1)(sum(comparison_2016_2021[1:2,5])/sum(comparison_2016_2021[1:2,3])-1)` increase, compared to the population increase of `r label_percent(accuracy=0.1)(sum(comparison_2016_2021[,5])/sum(comparison_2016_2021[,3])-1)` across all of Greater Melbourne.
...
There is a significant and strong correlation between the 2016 and 2021 SI scores(`r prmisc::print_cortest(cor.test(si_by_area_2016_2021_wider$x2016, si_by_area_2016_2021_wider$x2021))`).
The average SI value increased from `r si_by_area_2016_2021_wider$x2016 %>% mean() %>% format(big.mark = ",", digits = 5)` to  `r si_by_area_2016_2021_wider$x2021 %>% mean() %>% format(big.mark = ",", digits = 5)`(`r percent_format(accuracy = 1)(si_by_area_2016_2021_wider$x2021 %>% mean()/si_by_area_2016_2021_wider$x2016 %>% mean()-1)`.
...
```{r Greater_Melbourne_2016_2021_scatterplot_SA12021_appendix, fig.show="hold", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, include = TRUE, fig.fullwidth = TRUE, fig.height = 8, fig.cap="Greater Melbourne: SI scores in 2016 and 2021 by SA1 (2021 boundaries), adjusted to set those <1 equal to 1 (for visual clarity)"}
