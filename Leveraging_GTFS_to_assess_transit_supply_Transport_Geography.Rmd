...
Where comparisons are made to the @currie2010identifying results 
the same Census Collection Districts (CCDs) are used. 
...
```{r run_for_all_modes_Victoria_210810_SA1, eval = FALSE, echo = FALSE}
...
write.csv(si_CCD2006_21_census_week_aggregated, "results/si_CCD2006_21_census_week_aggregated.csv")
...
### SI scores
```{r Greater_Melbourne_2016_2021, fig.show="hold", echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, fig.cap="Transport Supply by SA1, weeks starting the dates of the 2016 and 2021 censuses"}

###2021
###Load results from CSV (Precalculated above) and make first column character
si_SA12021_21_census_week_aggregated <- read.csv( "results/si_SA12021_21_census_week_aggregated.csv") %>% 
  as_tibble()
si_SA12021_21_census_week_aggregated <- si_SA12021_21_census_week_aggregated[,2:3]

si_SA12021_21_census_week_aggregated$area_id <- si_SA12021_21_census_week_aggregated$area_id %>% 
  as.character()

##Join to absmaps so as to identify (and enumerate as zero) those SA1 zones 
# that have no SI score
si_SA12021_21_census_week_aggregated <- full_join(
  absmapsdata::sa12021 %>% 
    filter(gcc_name_2021 == "Greater Melbourne") %>% 
    select(sa1_code_2021) %>%
    st_drop_geometry(),
  si_SA12021_21_census_week_aggregated, 
  by = join_by(sa1_code_2021  == area_id))
si_SA12021_21_census_week_aggregated[is.na(si_SA12021_21_census_week_aggregated)] <- 0
...
sa_by_area_dataframe_non_zero <- cbind(
  sa_by_area_dataframe %>% 
    filter(SI != 0), 
  cuts)

#join back to dataframe, and put "Zero" in N/A
sa_by_area_dataframe <- full_join(
  sa_by_area_dataframe, 
  sa_by_area_dataframe_non_zero[,-2]
)
sa_by_area_dataframe$cuts <- sa_by_area_dataframe$cuts %>% 
  fct_explicit_na(na_level = "Zero Supply") %>% 
  fct_shift(n = -1)

names(sa_by_area_dataframe) <- c("sa1_code_2021", "SI", "Transit Supply")
...
si_SA12021_21_census_week_aggregated_thresholds <- set_thresholds(sa_by_area_dataframe = si_SA12021_21_census_week_aggregated)
si_SA12021_21_census_week_aggregated <- si_SA12021_21_census_week_aggregated_thresholds[[1]]
si_SA12021_21_census_week_aggregated$Year <- 2021
...
Figure \ref{fig:Greater_Melbourne_2016_2021} shows SI values on census day for Greater Melbourne in 2016 and 2021. Comparison to Figure \ref{fig:Currie_map_SI} indicates that. The transit network and the Greater Melbourne Greater Capital City Statistical Area (GCCSA) both cover more area than they did in 2006. However, there are still many areas, especially in the outer suburbs, with very low or zero transit supply. 

Higher levels of transit continue to be mostly in the inner and middle suburbs, and along railway corridors. However, there appears to be higher levels of service further out, especially close to the ends of most of the train lines.  As well, there are new isolated clusters of above average, high and very high levels of service, for example in the south-west (around Werribee area). However, some caution may be required in directly comparing the 2016 and 2021 maps with the 2006 map, due to the 
