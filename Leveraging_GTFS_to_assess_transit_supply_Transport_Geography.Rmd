...
names(sa1_irsad_2016) <- c("sa1_code_2016_7_digit", 
                           "sa1_code_2016", 
                           "usual_resident_population", 
                           "SEIFA_score_2016",
                           "blank",
...
sa1_students_2016 <- read_csv("data/sa1_students_2016.csv", 
    skip = 8)
...
#not_in_labour_force csv from ABS repeats the entire dataset, hence only read in first set
sa1_not_in_labour_force_2016 <- sa1_not_in_labour_force_2016 %>% filter(row_number() <= n()-61844)
...
#this dataset contains a single 'wafer', 
# being those people living in a household with no car, separated by age and sa1
sa1_no_car_2016 <- read_csv("data/sa1_no_car_2016.csv", 
    skip = 8)
...
sa1_low_income_2016 <- read_csv("data/sa1_low_income.csv", 
    skip = 8)
sa1_low_income_2016 <- sa1_low_income_2016[-1,]
sa1_low_income_2016 <- sa1_low_income_2016 %>% filter(row_number() <= n()-5)

#read disability support pension in 2016 and remove non-data rows
sa1_disability_support_pension_2016 <- read_csv("data/sa1_disability_2016.csv", 
    skip = 8)
sa1_disability_support_pension_2016 <- sa1_disability_support_pension_2016[-1,]
sa1_disability_support_pension_2016 <- sa1_disability_support_pension_2016 %>% filter(row_number() <= n()-5)




#read age in 2016 and remove non-data rows
sa1_age_2016 <- read_csv("data/sa1_age.csv", 
    skip = 8)
...
#calculate number of persons 5 to 9 years old.
sa1_age_2016$persons_5_to_9 <- rowSums(sa1_age_2016[,7:11]) 
#move age-related indicators into social indicators table. 
social_2016_sa1 <- sa1_age_2016 %>% 
  select(`AGEP Age`, over_60, persons_5_to_9)
names(social_2016_sa1) <- c("sa1_code_2016", "over_60", "five_to_nine")
...
#join disability pension 
sa1_disability_support_pension_2016 <-  sa1_disability_support_pension_2016 %>% 
  clean_names()
  
social_2016_sa1 <- full_join(social_2016_sa1, 
                             sa1_disability_support_pension_2016 %>% 
                               select(
                                 "igap_main_type_of_personal_government_benefit_payment_administrative_data", 
                                      "disability_support_pension"),
                             by = join_by(
                               "sa1_code_2016" == "igap_main_type_of_personal_government_benefit_payment_administrative_data"))
...
#join students
sa1_students_2016$students <- sa1_students_2016$Total
social_2016_sa1 <- full_join(social_2016_sa1, 
                             sa1_students_2016 %>% 
                               select("STUP Full-Time/Part-Time Student Status", "students"), 
...
#join IRSAD scores
sa1_irsad_2016$x2016_statistical_area_level_1_sa1 <- sa1_irsad_2016$x2016_statistical_area_level_1_sa1 %>% 
  as.character()
...
```{r Greater_Melbourne_2021_needs_gap, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, fig.height=8, fig.cap="Greater Melbourne 2021, SI and Combined Needs Index scores"}
...
ggplot()+ 
  geom_sf(data=si_by_area_2016_2021_wider %>% na.omit(),
...
```{r Greater_Melbourne_2016_social_needs, out.width="90%", echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE,  fig.cap="Distribution of categories of composite social need index scores in 2016 and 2021, overlayed with: 2006 Greater Melbourne boundary (black); middle/outer and inner/middle suburb boundaries (grey); and suburban railway lines (dashed)."}
