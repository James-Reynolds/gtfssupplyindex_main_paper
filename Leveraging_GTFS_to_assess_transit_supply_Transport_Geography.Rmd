...
```{r Melbourne_map, crop = TRUE, fig.cap = "SA1 zone boundaries in central Melbourne.",  echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, out.width='100%'}
...
```{r mornington_map_ABS, crop = TRUE, fig.cap = "SA1 zones and location of Mornington Tourist Railway Stations.",  echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, out.width='100%'}
...
#  head() %>%
#  adorn_rounding() %>%
#  kable(caption = "Mornington Penninsula Tourist Railway hourly SI values for December 30, 2018")
...
```{r Melbourne_CBD_map_230808, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.fullwidth = TRUE, crop = TRUE, fig.cap="Victorian GTFS and central Melbourne SA1 zones, SI values for October 10, 2023, by hour between 5am and 11am (top) and by mode (bottom)"}
...
```{r Melbourne_230808_by_hour, crop = TRUE, fig.cap = "Victorian GTFS and SA3 zones in Melbourne - Inner, SI values for Tuesday August 8, 2023, by hour, 5am to midnight", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}


si_by_area_and_hour_230808$hour_starting <- si_by_area_and_hour_230808$hour_starting %>% hm() %>% hour()


#Join SI to ABS names data
si_by_area_and_hour_230808  <- left_join(
...
si_by_area_and_hour_230808$hour_starting <- as.numeric(si_by_area_and_hour_230808$hour_starting)

si_by_area_and_hour_230808_sa3 <- aggregate(
  si_by_area_and_hour_230808$SI,
  list(si_by_area_and_hour_230808$hour_starting,
       si_by_area_and_hour_230808$sa3_name_2021, 
       si_by_area_and_hour_230808$sa4_name_2021),
  FUN = sum) %>%
  as_tibble()
names(si_by_area_and_hour_230808_sa3) <- c("hour_starting", "sa3_name_2021", "sa4_name_2021", "SI")

ggwithinstats(
  data = si_by_area_and_hour_230808_sa3 %>% 
    filter(sa4_name_2021 == "Melbourne - Inner") %>% 
    filter(hour_starting > 4 & 
            hour_starting < 24),
  x = hour_starting,
  y = SI,
  ...
  label.expression = SI < 100, ## expression for deciding which points to label
...
```
```{r Mornington_230808_by_hour, crop = TRUE, fig.cap = "Victorian GTFS and SA3 zones in Mornington Peninsula, SI values for Tuesday August 8, 2023, by hour, 5am to midnight", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r ggbetween_stats_greater_melbourne_230808_by_sa3, crop = TRUE, fig.cap = "Victorian GTFS and SA1 zones within Greater Melbourne, SI values for Tuesday August 8, 2023", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, eval = FALSE}
...
ggbetweenstats(
  data = si_by_area_230808,
  x = sa3_name_2021,
  y = SI,
  #type = "nonparametric", ## type of statistical test
  xlab = "SA 4 zone", ## label for the x-axis
  ylab = "SI", ## label for the y-axis
  package = "yarrr", ## package from which color palette is to be taken
  palette = "info2", ## choosing a different color palette
  pairwise.display = "significant"
  ...
  facet_wrap(sa4_name_2021, ncols =1)

```

```{r map_by_sa3, crop = TRUE, fig.cap = "Victorian GTFS and SA1 zones within Greater Melbourne, SI values for Tuesday August 8, 2023", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r gghistostats_greater_melbounre_230808, crop = TRUE, fig.cap = "Victorian GTFS and SA1 zones within Greater Melbourne, SI values for Tuesday August 8, 2023", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
names(si_by_area_230808) <- c("area_id", "SI", "Population")


ggscatterstats(
  data = si_by_area_230808, ## data frame from which variables are taken
  ...
  label.var = sa4_name_2021, ## variable to use for labeling data points
  label.expression = Population > 100 & SI < 100, ## expression for deciding which points to label
  point.label.args = list(alpha = 0.7, size = 4, color = "grey50"),
...
ggplot(si_by_area_230808 %>% select(SI, Population), 
  ...
  annotate_ineq(data_ineq = si_by_area_230808$SI, 
                freq_ineq = si_by_area_230808$Population,
                measure_ineq = "Gini", 
                color = "red",
...
```{r gghistostats_greater_melbourne_230808_by_sa3, crop = TRUE, fig.cap = "Victorian GTFS and SA1 zones within Greater Melbourne, SI values for Tuesday August 8, 2023", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
#Join SI to map data
map_si_by_area_230808 <- inner_join(map_areas_of_interest_sa3, si_by_area_230808)


#aggregate SI by SA3
map_si_by_area_230808_sa3 <- map_si_by_area_230808 %>%
  aggregate(SI ~ sa3_name_2021, FUN = sum) %>%
  as_tibble()
...
ggscatterstats(
  data = map_si_by_area_230808_sa3, ## data frame from which variables are taken
  x = Population, ## predictor/independent variable
  y = SI, ## dependent variable
  xlab = "Population", ## label for the x-axis
  ylab = "SI score", ## label for the y-axis
...
```{r Gini_coefficients_sa4, crop = TRUE, fig.cap = "Victorian GTFS and SA1 zones within Greater Melbourne, SI values for Tuesday August 8, 2023, Lorenz plot", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}

ggplot(map_si_by_area_230808 %>% select(SI, Population), 
       aes(x = SI, n = Population)) + 
         stat_lorenz()  +
  geom_abline(linetype = "dashed") +
  theme_bw() +
  ...
  annotate_ineq(data_ineq = si_by_area_230808$SI, 
                ...
                size = theme_get()$text[["size"]] / 2,
                fontface = "italic") +
  facet_wrap(sa4_name_2021, ncol = 1) 
