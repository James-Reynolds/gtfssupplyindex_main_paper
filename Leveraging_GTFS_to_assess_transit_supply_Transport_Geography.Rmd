...
p1 <- ggplot() + 
  geom_sf(data=map_si_by_area_and_hour_230808 %>% filter(hour_starting %in% (8:10)),
          aes(fill = SI)) +
            facet_wrap(vars(hour_starting), ncol=1) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
        axis.ticks.y=element_blank()  #remove y axis ticks
...
```{r Melbourne_230808_by_hour, crop = TRUE, fig.cap = "Victorian GTFS and SA3 zones, SI values for Tuesday August 8, 2023, by hour, 5am to midnight", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
si_by_area_and_hour_230808_sa3$hour_starting <- hm(si_by_area_and_hour_230808_sa3$hour_starting)
si_by_area_and_hour_230808_sa3$hour_starting <- hour(si_by_area_and_hour_230808_sa3$hour_starting)
...
```{r map_greater_melbourne_230808_by_sa3, crop = TRUE, fig.cap = "Victorian GTFS and SA3 zones within Greater Melbourne, SI values for Tuesday August 8, 2023 (top), 2021 census population (middle), and SI divided by population (bottom)", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r greater_melbourne_230808_by_sa3_scatterplot, crop = TRUE, fig.cap = "Victorian GTFS and SA3 zones within Greater Melbourne, SI values for Tuesday August 8, 2023 (top), 2021 census population (middle), and SI divided by population (bottom)", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
si_by_area_and_hour_230808_sa3  <- left_join(
  ...
  (absmapsdata::sa12021 %>% 
     filter(gcc_name_2021 == "Greater Melbourne") %>% 
     select(sa3_code_2021, sa3_name_2021, sa4_name_2021) %>%
     st_drop_geometry()), 
  by = join_by(area_id == sa1_code_2021)
)
