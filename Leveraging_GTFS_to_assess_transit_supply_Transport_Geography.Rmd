...
sa1_not_in_labour_force_2021 <- sa1_not_in_labour_force_2021 %>% filter(row_number() <= n()-5)
...
```{r Greater_Melbourne_2021_social_needs, fig.show="hold", echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, fig.cap="SI scores, census day 2021"}
...
cuts <- social_2021_sa1_greater_melbourne %>% 
  st_drop_geometry() %>% 
  filter(SI != 0) %>% 
  select(SI) %>% 
  unlist()  %>% 
