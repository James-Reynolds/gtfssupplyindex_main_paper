...
```{r run_for_all_modes_Greater_Melbourne_230808, eval = FALSE, echo = FALSE}
#eval is false, so as to not overwrite saved file
...
```{r greater_melbourne_230808_8am_versus_total_scatterplot_by_hour, crop = TRUE, fig.cap = "Victorian GTFS and SA3 zones within Greater Melbourne (except Melbourne City), SI values for Tuesday August 8, 2023 8 to 9am versus all day", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r Melbourne_City_sa2_230808_by_hour_per_resident, crop = TRUE, fig.cap = "Victorian GTFS and SA3 zones, SI per person values for Tuesday August 8, 2023, by hour, 5am to midnight", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}



#join_to_SI_scores
si_by_area_and_hour_230808_pop  <- left_join(si_by_area_and_hour_230808,
                               (X2021Census_G01_VIC_SA1 %>% 
                                  select(SA1_CODE_2021, Tot_P_P)), 
                                by = join_by(area_id == SA1_CODE_2021)
                               )
names(si_by_area_and_hour_230808_pop) <- c("area_id", "SI", "hour_starting", "Population")

si_by_area_and_hour_230808_pop$SI_per_pop <- si_by_area_and_hour_230808_pop$SI / si_by_area_and_hour_230808_pop$Population

si_by_area_and_hour_230808_pop <- si_by_area_and_hour_230808_pop %>%
  select(area_id, SI, Population, hour_starting, SI_per_pop)

#Join SI to ABS names data
si_by_area_and_hour_230808_pop <- left_join(si_by_area_and_hour_230808_pop, 
                               (absmapsdata::sa12021 %>% 
                                 filter(gcc_name_2021 == "Greater Melbourne") %>% 
...
#aggregate SI by SA2 for Melbourne City only
si_by_area_and_hour_230808_sa2_pop <- si_by_area_and_hour_230808_pop %>% 
  filter(sa3_name_2021 == "Melbourne City") %>%
  aggregate(SI_per_pop ~ hour_starting+sa2_name_2021+sa3_name_2021,
  FUN = sum) %>%
  as_tibble()

#convert hour starting to numeric
si_by_area_and_hour_230808_sa2_pop$hour_starting_numeric <- hm(si_by_area_and_hour_230808_sa2_pop$hour_starting)
si_by_area_and_hour_230808_sa2_pop$hour_starting_numeric <- hour(si_by_area_and_hour_230808_sa2_pop$hour_starting_numeric)



ggwithinstats(
  data = si_by_area_and_hour_230808_sa2_pop %>% 
    filter_if(~is.numeric(.), all_vars(!is.infinite(.))) %>%
    filter(hour_starting_numeric > 5 & 
            hour_starting_numeric < 23),
  x = hour_starting_numeric,
  y = SI_per_pop,
  #type = "nonparametric", ## type of statistical test
  xlab = "Hour starting", ## label for the x-axis
  ylab = "SI per person", ## label for the y-axis
  pairwise.display = "none"
)  
...
```{r Melbourne_230808_by_hour_by_pop, crop = TRUE, fig.cap = "Victorian GTFS and SA3 zones (except Melbourne City), SI per person values for Tuesday August 8, 2023, by hour, 5am to midnight", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}


#aggregate SI by SA3
si_by_area_and_hour_230808_sa3_pop <- si_by_area_and_hour_230808_pop %>% 
  aggregate(Population ~ hour_starting+sa3_name_2021+sa4_name_2021,
  FUN = sum) %>%
  as_tibble()

si_by_area_and_hour_230808_sa3_pop <- left_join(
  si_by_area_and_hour_230808_sa3 %>% 
    select(!hour_starting), 
  si_by_area_and_hour_230808_sa3_pop, 
  by = "sa3_name_2021")

si_by_area_and_hour_230808_sa3_pop$SI_per_pop <- si_by_area_and_hour_230808_sa3_pop$SI / 
  si_by_area_and_hour_230808_sa3_pop$Population

ggwithinstats(
  data = si_by_area_and_hour_230808_sa3_pop %>% 
    filter_if(~is.numeric(.), all_vars(!is.infinite(.))) %>% 
    filter(sa3_name_2021 != "Melbourne City") %>%
    filter(hour_starting_numeric > 5 & 
            hour_starting_numeric < 24),
...
```{r greater_melbourne_230808_sa3_by_hour_per_pop_percentatages, crop = TRUE, fig.cap = "Victorian GTFS and SA3 zones within Greater Melbourne, SI values for Tuesday August 8, 2023, by hour as a percentage of the full day", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}

si_by_area_and_hour_230808_sa3_pop_wider <- si_by_area_and_hour_230808_sa3_pop %>%
  select(sa3_name_2021, hour_starting_numeric, SI_per_pop) %>%
  pivot_wider(names_from = hour_starting_numeric, values_from = SI_per_pop)


si_by_area_and_hour_230808_sa3_pop_wider_percentages <- si_by_area_and_hour_230808_sa3_pop_wider %>% 
  adorn_percentages() 

si_by_area_and_hour_230808_sa3_pop_percentages <- si_by_area_and_hour_230808_sa3_pop_wider_percentages %>% 
  pivot_longer(cols = 2:ncol(si_by_area_and_hour_230808_sa3_pop_wider_percentages), 
               names_to = "hour_starting", 
               values_to = "SI_per_pop")

si_by_area_and_hour_230808_sa3_pop_percentages$hour_starting <- si_by_area_and_hour_230808_sa3_pop_percentages$hour_starting %>% as.numeric()

ggwithinstats(
  data = si_by_area_and_hour_230808_sa3_pop_percentages, ## data frame from which variables are taken
  x = hour_starting, ## predictor/independent variable
  y = SI_per_pop, ## dependent variable
  xlab = "Population", ## label for the x-axis
  ylab = "SI per person", ## label for the y-axis
  label.var = sa3_name_2021, ## variable to use for labeling data points
  # label.expression = Population > 15000 & SI < 5000, ## expression for deciding which points to label
...
```{r greater_melbourne_230808_8am_versus_total_scatterplot_per_pop, crop = TRUE, fig.cap = "Victorian GTFS and SA3 zones within Greater Melbourne (except Melbourne City), SI per person for Tuesday August 8, 2023 8 to 9am versus all day", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}



ggscatterstats(
  data = si_by_area_and_hour_230808_sa3_pop_wider_percentages,
  x = `8`, ## predictor/independent variable
  y = `20`, ## dependent variable
  xlab = "8-9am", ## label for the x-axis
  ylab = "8-9pm", ## label for the y-axis
...
```{r greater_melbourne_230808_8am_versus_total_scatterplot_per_pop_2, crop = TRUE, fig.cap = "Victorian GTFS and SA3 zones within Greater Melbourne (except Melbourne City), SI values for Tuesday August 8, 2023 8 to 9am versus all day", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r greater_melbourne_230808_5pm_versus_total_scatterplot_per_pop, crop = TRUE, fig.cap = "Victorian GTFS and SA3 zones within Greater Melbourne (except Melbourne City), SI values for Tuesday August 8, 2023 5 to 6pm versus all day", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r greater_melbourne_230808_9pm_versus_total_scatterplot_per_pop, crop = TRUE, fig.cap = "Victorian GTFS and SA3 zones within Greater Melbourne (except Melbourne City), SI values for Tuesday August 8, 2023 9 to 10pm versus all day", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r gghistostats_greater_melbourne_230808_per_pop, crop = TRUE, fig.cap = "Victorian GTFS and SA1 zones within Greater Melbourne, SI values for Tuesday August 8, 2023", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r Gini_coefficients_per_pop, crop = TRUE, fig.cap = "Victorian GTFS and SA1 zones within Greater Melbourne, SI values for Tuesday August 8, 2023, Lorenz plot", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r map_greater_melbourne_230808_by_sa3_per_pop, crop = TRUE, fig.cap = "Victorian GTFS and SA3 zones within Greater Melbourne, SI values for Tuesday August 8, 2023 (left) and 2021 census population (right)", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r greater_melbourne_230808_by_sa3_scatterplot_per_pop, crop = TRUE, fig.cap = "Victorian GTFS and SA3 zones within Greater Melbourne, SI values for Tuesday August 8, 2023 and 2021 census population (left), and log(SI divided by population) (right)", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
