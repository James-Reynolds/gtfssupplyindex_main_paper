...
```{r mornington_map_ABS, fig.cap = "SA1 zones (red), location of Mornington Tourist Railway Stations (black) and boundary of zones within 800m catchment (blue)", fig.width = 3, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
Again, ABS data was used as the areas of interest. 
...
```{r calculate_stop_in_or_near_areas_verbose, echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE, out.width='100%', fig.keep = "first", fig.cap= "Step 3, stop catchments for the Mornington Penninsula Tourist Railway, showing intersections with SA1 zones"}
stops_in_or_near_areas <- gtfssupplyindex:::stops_in_walk_dist(
  list_gtfs = list_gtfs, 
  areas_of_interest = areas_of_interest,
  EPSG_for_transform = 28355, 
...
```{r Melbourne_231015_by_mode, fig.cap = "Victorian GTFS and SA1 zones near the Melbourne CBD, SI values, by mode, 2023", crop = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
### sum across hours
si_by_area_231010_all_modes_all_day <- si_by_area_and_hour_231010_all_modes %>%
  group_by(area_id, mode) %>%
  summarise(Freq = sum(SI))
si_by_area_231014_all_modes_all_day <- si_by_area_and_hour_231014_all_modes %>%
  group_by(area_id, mode) %>%
  summarise(Freq = sum(SI))
si_by_area_231015_all_modes_all_day <- si_by_area_and_hour_231015_all_modes %>%
  group_by(area_id, mode) %>%
  summarise(Freq = sum(SI))

si_by_area_231010_all_modes_all_day$date <- "10/10"
si_by_area_231014_all_modes_all_day$date <- "14/10"
si_by_area_231015_all_modes_all_day$date <- "15/10"

#merge 
si_by_area_2310_all_modes_all_day <- merge(si_by_area_231010_all_modes_all_day, si_by_area_231014_all_modes_all_day, all = TRUE)
si_by_area_2310_all_modes <- merge(si_by_area_2310_all_modes_all_day, si_by_area_231015_all_modes_all_day, all = TRUE)

names(si_by_area_2310_all_modes) <- c("area_id", "mode", "SI", "date")

#Join SI to map data
map_si_by_area_2310_all_modes_all_day <- inner_join(map_areas_of_interest, si_by_area_2310_all_modes)

# my_breaks = c(2, 10, 50, 250, 1250, 6000)

#plot bus, tram and rail by date
ggplot() + 
  geom_sf(data=map_si_by_area_2310_all_modes_all_day,
          aes(fill = (SI))) +
            facet_grid(date ~ mode) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
        axis.ticks.y=element_blank()  #remove y axis ticks
  ...
  coord_sf(xlim = xlim, ylim = ylim) 
#+  scale_fill_gradient(name = "SI", trans = "pseudo_log"),
...
```{r run_for_all_modes_Victoria_230808, eval = TRUE, echo = FALSE}
#eval is false, so as to not overwrite saved file
...
1. Download
https://transitfeeds.com/p/ptv/497/20220929/download
...
2. Write for loop to run analysis across all, using SA1 2021 data.
