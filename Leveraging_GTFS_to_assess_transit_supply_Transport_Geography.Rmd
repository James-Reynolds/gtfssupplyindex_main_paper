...
# Introduction
Providing at least some motorised mobility 
for those who cannot otherwise drive 
themselves is a motivation for
...
especially those seeking to improve vertical equity 
...
Previous research developed an approach for identifying 
spatial gaps in transit supply related to social needs for transport
and applied this to a case study of Melbourne in 2006 [@Currie2003Hobart, 
@Currie2004Gap; @Currie2007Identifying; @currie2010identifying].
...
about transit service provision.
However, tools
for using GTFS data 
to examine spatial 
patterns and gaps 
...
which are compared to the 2006 analysis reported in @currie2010identifying^[The wider programme of research includes examination of spatial gaps in other cities, so as to better understand whether patterns in Melbourne are representative of other places. However, this paper is limited to examining Melbourne only, and results for other cities will be reported elsewhere.].
...
There are 
many metrics 
available for assessing transit services, with examples including: those in 
...
Case research approaches can be particularly useful research questions are about 'how' or 'why', but when researchers do not have control of events (thereby limiting the ability to use experimental approaches)[@Yin2009aa]. In this research the questions relate to (1) how to use GTFS data to assess gaps between social needs and transit supply, and (2) how spatial patterns might have changed since those reported for 2006 in @currie2010identifying. 

Addressing the "duality criterion" is a key issue when using case research approaches, this being the need of the research to be seeking generalisable findings while grounded in the context of only a small number of cases [@Denscombe2007aa; @Ketokivi2014aa]. Here the approach taken has been to develop a package of tools 
...
@Currie2003Hobart and @Currie2004Gap studies of Hobart and Adelaide used earlier versions of the needs-gap assessment methodology. Hence, it is only possible to compare Melbourne in 2006 to the Melbourne of now using the developed code. While this study does seek to findings about changes in spatial patterns of social need and transit supply that are generalisable to more than just Melbourne, a lack of 2006 result for other cities may limit the extent to which these findings might be considered representative of changes in other places^[Other parts of this research programme will examine changes over the last 10-15 years in cities (where GTFS data is available). The focus of this paper, however, is on Melbourne.].
...
This was sourced via the strayr 
and absmapsdata packages [@r-strayr].


## Melbourne Case Study

The methodological literature provides guidance on case study selection and discussion of various theoretical sampling approaches, including sampling for critical, particularly revelatory and/or representative cases [@Eisenhardt1989aa; @Yin2009aa; @Denscombe2007aa; @Eisenhardt2007TBfC]. @Yin2009aa notes the selection of a case so as to allow longitudinal study, and it is this that is the primary reason for the selection of Melbourne for the research reported in this paper. 
This selection allows direct comparison with the 2006 results reported in @currie2010identifying so as to explore the extent to which spatial patterns of needs-gaps in transit supply have changed over the last 10 to 15 years. As such, SI scores where calculated 
using the same 
...
Statistical Area 1 (SA1) zones. As such, SI scores have also been calculated for SA1s to facilitate the use of ABS data. The same Transport Supply categorizations have been used as in @currie2010identifying, 
with those CCDs/SA1s that have above average SI scores split evenly into three groups, those that have below average SIs also split evenly into three groups, and those CCDs/SA1s with SI scores of zero placed in their own category. 
...
```{r Greater_Melbourne_CCD_2021, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.dim = c(8.5,8.5), fig.cap="Melbourne (2006 extents), Transport Supply by CCD, week starting the day of the 2016 (top) and 2021 (bottom) censuses, overlayed with inner/middle/outer suburban boundary (blue) and suburban railway lines (black)"}
...
```{r Greater_Melbourne_2016_2021_ratio_map, fig.show="hold", fig.dim = c(8,8),  echo = FALSE, warning=FALSE, message=FALSE, cache = TRUE, include = TRUE, fig.cap="Greater Melbourne: changes in SI between 2016 and 2021, by SA1, overlayed with 2006 Melbourne boundary (black), SA4 boundaries (blue) and suburban railway lines (black)"}
...
It is, however, possible to directly compare SI scores 2016 and 2021. Using all of Greater Melbourne and the 2021 definition of SA1s indicates that there is a significant and strong correlation between the 2016 and 2021 SI scores (`r prmisc::print_cortest(cor.test(si_by_area_2016_2021_wider$x2016, si_by_area_2016_2021_wider$x2021, method = "pearson"))`, `r prmisc::print_cortest(cor.test(si_by_area_2016_2021_wider$x2016, si_by_area_2016_2021_wider$x2021, method = "spearman"))`). Average SI scores increased from `r si_by_area_2016_2021_wider$x2016 %>% mean() %>% format(big.mark = ",", digits = 5)` in 2016 to `r si_by_area_2016_2021_wider$x2021 %>% mean() %>% format(big.mark = ",", digits = 5)` in 2021 (+`r percent_format(accuracy = 0.1)(si_by_area_2016_2021_wider$x2021 %>% mean()/si_by_area_2016_2021_wider$x2016 %>% mean()-1)`). Table \ref{tab:Greater_Melbourne_2016_2021_ratio_population_table} shows changes in SI score between 2016 and 2021 (column 1) by 2021 SA1s for the 2021 population, summarised for each SA4 zone (columns 2 to 10) and Greater Melbourne as a whole (column 11). These are mapped in Figure \ref{fig:Greater_Melbourne_2016_2021_ratio_map}. 
There is a statistically significant difference in the distribution of service level changes across the populations between SA4 areas (`r prmisc::print_chi2(greater_melbourne_ratio_population_wider %>% select(Inner, "Inner East", "Inner South", "North East", "North West", "Outer East", "South East", "West", "Mornington Peninsula")  %>% as.matrix())`). More than half of the population in the North-West, West, North East, South East, Mornington Peninsula and Inner SA4s had SI scores that were higher in 2021 by one or more percent than they had been in 2016. Service levels remained within one percent for `r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[8,6] %>% as.numeric() /sum(greater_melbourne_ratio_population_wider[,6]))` of the population in the Outer East and `r label_percent(accuracy = 0.1)(greater_melbourne_ratio_population_wider[8,2] %>% as.numeric() /sum(greater_melbourne_ratio_population_wider[,2]))`. More of the population had less service in 2021 than 2016 in the Inner (`r label_percent(accuracy = 0.1)(sum(greater_melbourne_ratio_population_wider[9:12,10]) /sum(greater_melbourne_ratio_population_wider[,10]))`), Inner South (`r label_percent(accuracy = 0.1)(sum(greater_melbourne_ratio_population_wider[9:12,3]) /sum(greater_melbourne_ratio_population_wider[,3]))`) and Outer East (`r label_percent(accuracy = 0.1)(sum(greater_melbourne_ratio_population_wider[9:12,6]) /sum(greater_melbourne_ratio_population_wider[,6]))`) than in other SA4s. In terms of overall population, however, the largest shares to have at least one percent more service in 2021 were in the west (`r sum(greater_melbourne_ratio_population_wider[2:7,8]) %>% format(big.mark = ",")` residents in 2021), South East (`r sum(greater_melbourne_ratio_population_wider[2:7,7]) %>% format(big.mark = ",")`) and the Inner (`r sum(greater_melbourne_ratio_population_wider[2:7,10]) %>% format(big.mark = ",")`) SA4 areas.  The largest shares of residents with at less one percent lower service level in 2021 were among the Inner (`r sum(greater_melbourne_ratio_population_wider[9:12,10]) %>% format(big.mark = ",")`), Outer East (`r sum(greater_melbourne_ratio_population_wider[9:12,6]) %>% format(big.mark = ",")`) and South East (`r sum(greater_melbourne_ratio_population_wider[9:12,7]) %>% format(big.mark = ",")`) SA4 areas.   
...
```{r Greater_Melbourne_2021_social_needs, fig.dim = c(8.5,8.5), echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE,  fig.cap="Distribution of categories of composite social need index scores, overlayed with: 2006 Greater Melbourne boundary (black); SA4 boundaries (blue); and suburban railway lines (dashed)."}
...
Figure \ref{fig:Greater_Melbourne_2021_social_needs} shows the distribution of categories of social need index scores across Greater Melbourne for 2021^[A map for 2016 is included in the Appendix as Figure \ref{Greater_Melbourne_2016_social_needs_appendix]. This is analogous to the 2006 map shown in Figure \ref{fig:Currie_map_SI} (middle), although as discussed in the methodology section above, it was not possible to exactly replicate the @currie2010identifying social needs scoring approach due to changes in the way census results are reported. In general, the spatial grouping of different levels of social need appears less consistent in 2021 than in 2006. However, this may be an artifact of the shift to SA1s from  CCDs^[CCDs were originally devised to group the approximately 200 dwellings allocated to each individual census collector, whereas SA1s were introduced to be consistent in population (200 to 800 people, averaging 400) and character [@ABS_SA1s_CCDs]].
...
```{r Greater_Melbourne_2021_needs_gap_scatterplot_figure, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, fig.height=8, fig.cap="Greater Melbourne 2016, SI and Combined Needs Index scores, with SI scores < 10 rounded up to equal 10. "}
...
```{r Greater_Melbourne_2021_needs_gap_zones_tables, echo = FALSE} 
...
```{r Greater_Melbourne_SA12016_plot_appendix, fig.show="hold", echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.dim = c(8.5,8.5), fig.cap="Greater Melbourne, Transit Supply by SA1 for the week starting the date of the 2016 census, overlayed with: 2006 Greater Melbourne boundary (black); 2021 SA4 boundaries (blue); and suburban railway lines (black)"}
